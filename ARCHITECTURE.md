...
Counting convention: a member is an active entry in the root `Cargo.toml` `workspace.members` list — 39 members over 40 `crates/` directories, the 40th being the parked doc-class tool `workflow-docs` (commented out of the workspace), which no domain row or tier counts.
...
| `surface-*`  | surface-syntax, surface-render-remote, surface-grammar, surface-parser, surface-lsp, surface-engine, surface-repl, surface-tui, surface-corpus, surface-driver                                                                                    | user-facing syntax and tools: CST + diffing, inspection wire protocol, grammar, parser, language-server face, lowering engine with kernel admission, read-evaluate loop, terminal face, example corpus, toolchain driver                                                            |
...
tier 9   runtime-ffi → core-sequent, core-term, runtime-effects, surface-engine
         ...
         surface-syntax
         surface-repl → surface-engine, surface-grammar, surface-parser,
...
tier 10  surface-corpus → core-sequent, core-term, runtime-effects, runtime-ffi,
         ...
         surface-tui → surface-repl, surface-render-remote, surface-syntax
off-tier workflow-gates, workflow-dylint — tooling; depend on no workspace crate
         ...
         surface-driver → core-term, runtime-ffi, surface-engine, surface-lsp,
         surface-repl, surface-tui — process entry point: script runner,
