...
Counting convention: a member is an active entry in the root `Cargo.toml` `workspace.members` list — 44 members over 45 `crates/` directories, the 45th being the parked doc-class tool `workflow-docs` (commented out of the workspace), which no domain row or tier counts.
