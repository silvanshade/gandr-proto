...
Counting convention: a member is an active entry in the root `Cargo.toml` `workspace.members` list — 41 members over 42 `crates/` directories, the 42nd being the parked doc-class tool `workflow-docs` (commented out of the workspace), which no domain row or tier counts.
...
```text
tier 0   kernel-strata · kernel-conversion-trace · storage-chunker · surface-syntax · surface-render-remote
