...
Counting convention: a member is an active entry in the root `Cargo.toml` `workspace.members` list — 33 members over 34 `crates/` directories, the 34th being the parked doc-class tool `workflow-docs` (commented out of the workspace), which no domain row or tier counts.
...
9. **The compilation host is discovered, not pinned, so the wall requires it conditionally and checks it unconditionally.** `runtime/compile-host/` builds against an installed MLIR whose version must match the compiling clang, and nothing in the Rust workspace links it — `runtime-compile-host` resolves the host's C boundary at run time, so a checkout with no MLIR still builds and tests everything here.
   The wall carries three things. `compile-host:wall` runs the host's own gates when a **usable** installation is present and prints a named skip when it is not (`GANDR_COMPILE_HOST_STRICT=1` turns the skip into a failure). `crates/runtime-compile-host/tests/contract.rs` holds the Rust mirror of the boundary — heap layout, cell and node numbering, constructor arities, wire version, ABI version and statuses, the boundary struct's field order, the verifier-first pipeline, and the grade operations' effect declarations — to the host's own sources, unconditionally. The residual gap is behaviour: no checkout without MLIR proves the host *does* what its declarations say, and closing that means pinning MLIR.
