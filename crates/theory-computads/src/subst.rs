...
//!   *application* ([`crate::rewrite`]) runs. Non-linear patterns are admitted
//!   (VDC addendum §12.4): a repeated metavariable must bind structurally-equal
//!   subterms, checked by [`Eq`].
//! - [`unify_cmd`] is **two-sided**: two cell patterns (metavariables on both
//!   sides, kept apart by [`crate::overlap`]'s renaming) unified into a most
//!   general unifier — the superposition step of overlap enumeration (§7.3.2).
//!
