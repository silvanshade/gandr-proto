...
//! [`crate::tracelet::replay_equivalent`] — the semantic oracle — reads only
//! the boundary and whether the paths replay. A relation that reads a
//! derivation's steps therefore separates certificates the oracle identifies,
//! and no amount of structure here changes that. The event order is a
...
//! conjunct answers [`crate::alphabet::PositionOrder::Incomparable`] for a pair
//! exactly when it does for the swap, its second asks the overlap enumerator in
//! both ordered directions, and its third does not read the pair at all.
//!
...
//!
//! [`CellAlphabet::splice_cmd_at`]: crate::alphabet::CellAlphabet::splice_cmd_at
