...
//! crate provides the acceptance target.
//!
...
///   constructor (so return-side `K⁻` frames reduce) plus every
///   successfully-elaborated rule cell; `ops` holds one [`OpFrame`] per
///   operation of `desc.ops` the cell layer admits and `declined_ops` the rest,
///   both in declaration order; `declined_faces` pairs each declined face (by
///   index into `desc.cells`) with its [`ElaborateError`]. A face applying a
///   declined operation is itself declined
...
///   passes the operation gate here before [`elaborate_rule`] shapes it, so a
///   further admission condition is one more check at that seam.
/// - panics: none.
///
/// # Adequacy
...
pub fn elaborate_data_desc(desc: &DataDesc) -> DescElaboration
{
    ...
    for (index, face) in desc.cells.iter().enumerate() {
        ...
        // decided here, before `elaborate_rule` shapes it.
        let elaborated = match declined_operation(face, &unrepresentable) {
            | Some(error) => Err(error),
            | None => elaborate_rule(face),
        };
        match elaborated {
            | Ok(cell) => {
                store.insert(cell);
            },
            | Err(error) => declined_faces.push((DeclinedFaceIndex::from(index), error)),
        }
    }
    DescElaboration {
        store,
