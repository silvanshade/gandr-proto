...
//! enumeration (`crate::overlap`) unifies a subterm at a [`Pos`] and splices
//! the reduct back with [`splice_at`]. Position paths are bounded by pattern
//! depth (rule sizes are author-controlled and the generated ground configs are
//! depth-capped), and the navigation recurses only on the *path*, not on
//! user-sized input.
...
/// ([`crate::elaborate`]) maps a [`gandr_theory_levitation::FreeTerm::Ctor`] /
