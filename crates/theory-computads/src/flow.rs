...
//! [`crate::tracelet::replay_equivalent`] remains the identity criterion and
//! [`crate::normal_form::nf_equal`] remains the decidable fast path. This
//! module exists to **measure** one question: whether tracking cell and atom
//! occurrences across a recorded leg — and forgetting the formula-level
//! arrangement — yields a relation that coincides with certificate identity.
//! It answers by disagreeing with both neighbours on recorded fixtures, and the
//! disagreements are the deliverable; see the differential suite (`flow::tests`
//! in the crate's integration target).
//!
...
//! [`crate::sequent::SequentAlphabet`] a command pattern is one cut whose
//! children are a producer and a consumer, so a term has exactly one address
//! and every leg over it projects to a flow whose every thread runs from `⊤` to
//! `⊥` through one vertex. The differential suite therefore runs on the toy
//! alphabet, the one alphabet in the tree whose terms nest commands.
//!
...
//! [`crate::rewrite::CellApp`] has no room for. The strict projection is
//! sound without it — refusing to identify two occurrences is always the safe
//! direction — and it is exact on the permutation-tile class, where the two
//! images are disjoint and nothing needs transporting. It is *coarser than the
//! definition* only outside that class.
//!
...
//!   [`crate::overlap::overlaps_between`], which is a question about the
//!   alphabet. The projection reads the *instance*, where two disjoint images
//!   share no occurrence whatever the cells could have done elsewhere, so it
//!   licenses that pair.
//! - **Flow equality** is strictly inside replay-equivalence.
//!   [`crate::tracelet::replay_equivalent`] compares a boundary and asks each
//!   side to replay, ignoring the recorded paths outright, so no relation that
//!   reads the paths can coincide with it. [`crate::tracelet::derive_fused`]
//!   exhibits the gap in the tree: one boundary whose `path_a` is a two-step
//!   derivation and whose `path_b` is a single fused step — replay-equal by
//!   construction, and flow-distinct because the vertex label multisets differ.
//!
...
//!
//! [`CellAlphabet::command_positions`]: crate::alphabet::CellAlphabet::command_positions
//! [`ConvexityDischarge::LeftConnectedOverAcyclicTarget`]: crate::alphabet::ConvexityDischarge::LeftConnectedOverAcyclicTarget
//! [`ConvexityDischarge::ReCheckRequired`]: crate::alphabet::ConvexityDischarge::ReCheckRequired
...
/// [`Tracelet::replay`]: crate::tracelet::Tracelet::replay
#[inline]
