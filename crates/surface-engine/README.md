...
- **Diagnostics.** One versioned report joins typing failures, hole goals, and entity attributes into the envelope the inspection surface projects from; every diagnostic carries the shared stable code and typed message arguments.
...
- **The description route and the matcher seam.** A `data`, `codata` or `sign` block elaborates to a description, and the description elaborates into the cell engine's store: a frame cell per constructor, a cell per admitted `rule` member, derived cells from admitted circuit overlaps, and the η cell a declaration licenses.
  This crate is the **supply point** where circuit-algebra embedding matches reach completion: an admitted embedding is elaborated and instantiated through `gandr-theory-computads` into a candidate ordinary sequent `Overlap`, then the supplied-overlap boundary validates it before derived work begins.
  Boundary validation can decline a non-unifying substitution, and surface retains typed declines for embeddings that cannot be faithfully represented, so admission alone does not guarantee completion.
  `DescCells` retains the structured `CircuitCompletion` beside each store, including `Completed`/`Declined`, pending overlap batches, decline reasons, derived cells, certificates, embedding origins, declaration-origin side-map entries, and replay counts.
  Structural `CellId` deduplication remains the cell store's rule.
