...
pub fn elaborate_desc_cells(descs: &[DataDesc]) -> DescCells
{
    ...
    for desc in descs {
        ...
        for &(index, error) in &elaborated.declined_faces {
            let span = desc
                .cells
                .get(usize::from(index))
            ...
            cells.diagnostics.push(ElabDiagnostic::new(
                format!(
                    "rule of `{}` {}",
                    desc.id.name.as_ref(),
                    face_decline_reason(error).as_ref()
                ),
                span,
            ));
        }
...
/// The human-readable reason a `rule` face earned its cell-layer decline.
fn face_decline_reason(error: ElaborateError) -> DeclineReason<'static>
{
    DeclineReason(match error {
        | ElaborateError::LhsNotOperation => {
            "does not rewrite an operation application on its left-hand side"
        },
        | ElaborateError::EmptyOperation => {
            "applies an operation to no arguments, so there is no matched producer to cut against"
        },
        | ElaborateError::UnsupportedShape => {
            "has a term shape outside the supported flattening fragment"
        },
        | ElaborateError::UnrepresentableOperation => {
            "applies an operation whose declared arity the cell layer declined"
        },
    })
}
