...
//! granularity the current (melder, tree-sitter-free) architecture exposes: the
//! **top-level item**. Items lower independently and are typed against an
//! accumulating [`Ctx`] a [`crate::session::Session`] threads item to item, so
//! the checkpoint of an item is its typing result plus its [`Footprint`] (the
//! context names it read, `crate::footprint`), and the §4 validity condition
//! specializes to:
//!
...
//! order. The dirty-frontier pass is then driven in that order (dependencies
//! flow forward, so one ordered pass propagates every binding change), the
