...
//! lowered term read** (its free variables). This is the §4 footprint
...
)
{
    match *comp {
        ...
        // footprint obligation `footprint` tracks; only the scrutinee value and
        // the body (under `fst_name`/`snd_name`) are walked.
        | Comp::Split {
            ref scrut,
            ref fst_name,
            ref snd_name,
...
mod tests
{
    ...
    {
        let footprint = footprint_of(&Term::Value(Value::hole(0_u32)));
