...
* The A2.3 incremental trio — `checkpoint`, `footprint`, and `region` — moved out to `gandr-core-incremental`, together with the four `boundary` wrappers only they used (`DefinitionName`, `MatchDecision`, `HolePresence`, `AdoptedItemCount`), the `gandr-theory-orders` dependency, and the `tests/incremental.rs` differential gate.
