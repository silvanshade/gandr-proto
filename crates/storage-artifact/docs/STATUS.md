...
* `ArtifactIdentity` is `BLAKE3` of the canonical manifest bytes — the b3sum-provenance successor.
  ...
  From B2.3 it is exercised end-to-end as artifact-level provenance by the export exit gate (`gandr-core-sequent`'s `kernel_export_gate`, `GANDR_BLESS_KERNEL_EXPORT`): each S1-eligible corpus item and kernel-native golden records its identity, so a change to any item's canonical artifact bytes changes its recorded identity.
