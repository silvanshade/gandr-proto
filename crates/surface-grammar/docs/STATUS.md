...
* Modules — `check` (the cross-rule PBG checks: unique tiles, operator form, assumption-3 adjacency), `model` (`Pbg`, `Rule`, `PrecTable`, `SurfaceForm`, `Tile`, the `Adaptation` surface, and the `MoldId`/fingerprint model), `mold` (the `MoldDef` regex-zipper table and reachable-context steps), `walk` (the generative walk-index front-end over the delivered walk engine — `walk_index`, `comparison_table`, `reachable_molds`, `seen_key_verdict`), `surface` (`built_in`/`built_in_prec_table`, `PBG_ONLY_KINDS`, `TREE_SITTER_NAMED_KINDS`, and the `term`/`type_shell` submodules that assemble the full surface grammar), `highlight` (the normative mold highlighter: `highlight`, `role_of`), and `parity` (the standalone tree-sitter named-kind provenance inventory: `NamedKind`, `named_kind_parity`, `named_kind_realization`).
...
* The deliberate DEV-only dependency on the parser (the wyrd cycle break — the parser depends on the grammar, so the reverse edge can only be a dev edge) is NOT carried: `surface-parser` does not exist until rung F2.
  The parser-coupled surface-acceptance contracts are parked and return at F2, which re-adds the dev-dependency:
  + `tests/surface.rs` `contracts` module (the whole suite is coupled to the parser through a module-level `use gandr_parser::parse`): `named_kind_coverage_is_semantic`, `built_in_precedence_bands_are_exact`, `right_associative_type_operator_chains_parse_cleanly`, `mixed_set_type_operators_require_parentheses`, `cyclic_named_precedence_spec_reports_closed_named_witness`, `built_in_adaptations_name_concrete_rules_without_relaxing_checks`.
...
* The `tests/surface.rs` `contracts` suite is parked to F2 as a whole because its parser import is module-level; four of its six tests (`named_kind_coverage_is_semantic`, `built_in_precedence_bands_are_exact`, `cyclic_named_precedence_spec_reports_closed_named_witness`, `built_in_adaptations_name_concrete_rules_without_relaxing_checks`) are in fact parser-free and could be salvaged into F1 for extra unit-golden coverage if the owner prefers that over keeping the suite intact for its F2 return.
