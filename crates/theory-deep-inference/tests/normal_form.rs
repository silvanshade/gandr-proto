...
//! [`gandr_theory_deep_inference::shift`] runs there: a sequent command pattern
//! is one cut whose children are a producer and a consumer, so a sequent term
//! has exactly one command position and no two applications can ever be
...
//!
//! The oracle is [`gandr_theory_coherent_resolutions::replay_equivalent`] — the
//! engine's own identity criterion, which re-executes both derivations by
//! ground rewriting. It is external to the code under test: nothing in
//! [`gandr_theory_deep_inference::normal_form`] participates in the answer, so
//! a mutant that shifts both the normal form and the oracle identically cannot
...
//!
//! - inside [`gandr_theory_deep_inference::normal_form::normalize`], which
//!   replays its own canonical schedule and refuses with
//!   `ShiftedScheduleDoesNotFire` / `ShiftedScheduleMissesTheJoin` — these
//!   properties assert the normalization succeeds, so either refusal fails the
//!   suite with the offending schedule attached;
...
//! extension of is its own surface ([`gandr_theory_deep_inference::causal`]),
//! and it is exercised here rather than in a suite of its own because it needs
//! exactly the toy fixtures above: a sequent term has one command position, so
//! every sequent-side derivation is a chain and the order degenerates.
//!
...
mod tests
{
    ...
    use gandr_theory_cell_complexes::CausalDepth;
