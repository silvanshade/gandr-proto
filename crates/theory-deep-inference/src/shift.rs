...
//!    ([`gandr_theory_coherent_resolutions::overlap::overlaps_between`]);
...
//!
//! Over [`gandr_theory_cell_complexes::sequent::SequentAlphabet`] a command
//! pattern is a single cut whose children are a producer and a consumer, so the
//! *only* command position in a term is the root: two applications can never be
//! incomparable, and the shift quotient's extension over the sequent alphabet
//! is empty. The guard is therefore a specification the alphabet has not yet
//! reached rather than a patch to an existing fast path, exactly as the spike
//! recorded — and it is live the moment an alphabet nests commands, which is
//! what a circuit-shaped body does.
//!
...
use gandr_theory_coherent_resolutions::overlap::overlaps_between;
use gandr_theory_coherent_resolutions::rewrite::CellApp;
use gandr_theory_coherent_resolutions::rewrite::rewrite_at;
use gandr_theory_coherent_resolutions::tracelet::replay_from_peak;
...
impl<A: CellAlphabet> ShiftEquivalence<A>
{
    ...
    ///
    /// This is the tracelet replay criterion
    /// ([`gandr_theory_coherent_resolutions::tracelet`]) read on a
...
mod tests
{
    ...
    use gandr_theory_cell_complexes::alphabet::path_order;
