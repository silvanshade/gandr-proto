...
//! closed: [`gandr_theory_coherent_resolutions::tracelet::replay_equivalent`]
//! remains the semantic oracle and certificate identity is unchanged by this
//! module. The witness that the asymmetry is real, not a disclaimer, is
//! [`gandr_theory_coherent_resolutions::tracelet::derive_fused`]: it produces one boundary whose `path_a`
...
//! which is exactly
//! [`gandr_theory_coherent_resolutions::tracelet::replay_equivalent`], with no
//! appeal to a chain of commutations.
//!
...
use gandr_theory_cell_complexes::boundary::NormalFormEquality;
use gandr_theory_cell_complexes::boundary::PrimMultiplicity;
...
/// `gandr_theory_decomposition_spaces::transport::transport_step_id`; there is
/// no conversion between the two in either direction.
#[repr(transparent)]
...
/// exist: the transport boundary
/// (`gandr_theory_decomposition_spaces::transport`) addresses a **step** —
...
/// (`gandr_theory_decomposition_spaces::transport`).
#[repr(transparent)]
...
///   against `store` — the same triple
///   [`gandr_theory_coherent_resolutions::tracelet::Tracelet::replay`] would be
///   given.
...
///   converse does not hold and is not claimed.
...
///   [`gandr_theory_coherent_resolutions::tracelet::replay_equivalent`] holds
///   of any two tracelets carrying these boundaries and paths. A negative
///   answer means **nothing**: two replay-equal derivations with different
///   primitive factorizations are NF-distinct by design.
/// - panics: none.
///
/// # Adequacy
...
///   [`gandr_theory_coherent_resolutions::tracelet::replay_equivalent`] holds;
///   a negative answer means nothing. What is added is that the positive answer
///   no longer rests on a caller's word about where its inputs came from.
/// - panics: none.
///
/// # Adequacy
...
///   forces [`gandr_theory_coherent_resolutions::tracelet::replay_equivalent`].
...
///   ([`gandr_theory_coherent_resolutions::tracelet::replay_equivalent`]) and
///   the differential asserts the implication over generated derivation pairs
///   rather than over hand-picked ones. The L3 residue is the two negative
///   directions the generator cannot reach, because it only ever generates
///   certificates that normalize and agree: the *direction* an obstruction
///   collapses in, separated by a certificate whose recorded step does not
///   fire, and the conjunction over both legs, separated by a pair that agrees
///   on `path_a` and not on `path_b`.
...
mod tests
{
    ...
    use gandr_theory_coherent_resolutions::tracelet::replay_equivalent;
