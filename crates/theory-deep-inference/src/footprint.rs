...
//! [`gandr_theory_cell_complexes::boundary::FiringPermission`] says whether a
//! cell's provenance permits it to fire at a target term — an η-polarity
//! admissibility flag, with no relation to ownership or to a permission monoid.
//!
//! # Interaction with the metavariable-seam gap
//!
//! The overlap enumerator counts a **metavariable position** as a composition
//! seam ([`gandr_theory_coherent_resolutions::overlap::overlaps_between`];
//! recorded as the independence relation's own gap on the tracker's
//! `cells_equal` tractability spike). Over an alphabet whose every subterm is a
//! command position, a cell whose right-hand side exposes a hole therefore
//! "overlaps" every cell, and the guard's second conjunct refuses the pair.
//!
...
//!
//! [`CellApp`]: gandr_theory_coherent_resolutions::rewrite::CellApp
