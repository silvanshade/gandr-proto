...
- `checkpoint` — the validated-resume engine.
  An item's cached typing is adopted exactly when its identity — name, ascription, and lowered term together — is unchanged, no name in its footprint changed binding, and nothing its typing could consult definitionally changed value.
  That last condition is what makes reuse sound for items whose typing reads a definition's _body_ rather than only its type: a definition contributes an unfolding rule as well as a binding, and comparing types normalizes them.
  It costs nothing for ordinary items, whose type support is empty, so a body edit that preserves a definition's type still leaves its dependents adoptable.
...
A cyclic definitional environment — which shadowing can build, since definitions are threaded in source order — makes the checker's conversion driver run unboundedly when an identity type's endpoints force it.
That is `gandr-bpci`, and it is not this crate's to fix; the differential's generator keeps definition names unique whenever it generates an identity ascription, so it cannot reach the combination, and that restriction lifts when the defect closes.
...
Every resume the differential performs discharges four obligations, so a fixed witness and a generated edit are held to one standard: the resumed typings equal a from-scratch re-type; every adopted item's recorded footprint matches an independent re-scan of its term and reads only names bound identically at the item's new and old positions, with an opaque footprint — which reads everything — held to agreement across the whole visible binding map; the resulting checkpoint set survives the persistence codec unchanged, and an edit sequence resumes from the _decoded_ set so persistence sits inside the loop; and there is one adoption flag per typing.
...
The `value_mediated_reads` module holds the regression cases for reads that reach a definition's value rather than its type, including one that reaches it through a definition the edit never touched.
