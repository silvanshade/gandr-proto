...
//! [`gandr_core_term::types::ValueType`], and [`decode_desc`]
...
//! structurally-`Eq` [`gandr_core_term::types::ValueType`], so
...
pub enum DecodeError
{
    ...
    /// [`gandr_core_term::types::ValueType::Data`] handle at the
    /// *pipeline* seam, which `gandr-theory-levitation` cannot mint (it
    /// holds no [`gandr_core_term::types::DataId`] allocator) — so
