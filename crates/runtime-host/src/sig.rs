...
//!
//! v0 is sound because `Σ` is vacuous and resumption is multi-shot (ADR-34):
//! the host is an always-resume ambient handler on the seam
//! ([`gandr_core_checker::host`], realized by the L machine
//! [`gandr_core_sequent::machine::run_comp_with_host`], ADR-35 D4).
//!
//! The source-level host-module surface (the `fs.read(…)`-style module calls
//! and the `#!{ … }` shell-block lowering that shared these signatures) returns
//! with the surface engine; while it is parked, the canonical definitions live
//! here rather than being re-exported from the (unported) lowerer.
//!
