...
//! instead exists and is `gandr_theory_decomposition_spaces::transport`:
//! canonical bytes for resolved cell content and application position, a
//! distinct fixed-width `TransportStepId` verified by exact-length ingest, and
//! — when a persistence consumer lands — resolution back to a local [`CellId`]
//! by content lookup before replay.
use alloc::vec::Vec;
