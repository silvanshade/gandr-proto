...
//! path (`gandr_theory_computads::elaborate`), on the cells a
...
//! (`gandr_theory_decomposition_spaces::compose::compose_directed`). That shape
//! is **not** a copy and is **not** refused: the copy relation is per `(name,
//! category)` pair, which is exactly [`crate::pattern::MetaVar`]'s own
//! equality.
...
/// before a cell enters a store;
/// `gandr_theory_computads::elaborate::elaborate_data_desc` is its caller
/// today. It rejects nothing that [`crate::sequent::CellMeta::derive`]
/// computes: metadata derivation and admission are deliberately separate, so
/// internally-constructed non-linear patterns (multi-sum contract witnesses,
/// unification goals) stay constructible.
///
/// # Contract
