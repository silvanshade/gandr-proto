...
//! [`gandr_theory_coherent_resolutions::tracelet::replay_equivalent`], ADR-69
//! D1).
//!
//! # The graph seam
//!
...
//! [`gandr_theory_coherent_resolutions::tracelet::replay_equivalent`] — a peak,
//! a join, and two replays — which forgets the recorded derivation entirely. So
//! two presentations of one certificate can carry different supports, and **the
//! verdict can differ between them**. That is measured rather than feared:
...
use gandr_theory_cell_complexes::alphabet::SeamRole;
use gandr_theory_cell_complexes::boundary::VarianceFlowRole;
...
/// sequent alphabet
/// [`gandr_theory_cell_complexes::sequent::CellMeta::invertible`]) is the
/// caller's warrant that both certificates live in the groupoid fragment.
///
/// # Contract
...
/// [`gandr_theory_coherent_resolutions::tracelet::replay_equivalent`] compares.
/// So two presentations of one certificate compose to two presentations of one
/// certificate, and the operation **descends to the replay quotient** — which
/// is what the directed lane's verdict does not do (see the module header).
///
/// # Adequacy
...
/// support, which
/// [`gandr_theory_coherent_resolutions::tracelet::replay_equivalent`] forgets.
/// The module header states what that costs and why it is carried rather than
/// repaired here; the short form is that a decline is a fact about the
/// derivation in hand, the `Ok` branch's composite is an invariant even so, and
