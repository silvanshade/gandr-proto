...
* **Tree-sitter node-identity resync.** When this structure is wired into the pipeline, an incremental reparse that shifts byte ranges but preserves subtrees must re-sync order points without re-inserting unchanged elements.
