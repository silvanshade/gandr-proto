...
The toolchain's process boundary: the `gandr` binary, and the one face of it that exists today, the script runner.

The driver itself decides almost nothing.
It reads the command line, hands one source path to the engine's combined native and shell host, renders a returned value, and turns the run's outcome into a process exit status.
...
- The `tui`, `lsp`, `mcp`, `fmt` and `build` faces.
