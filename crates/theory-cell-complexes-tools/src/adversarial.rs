//! **Adversarial [`CellAlphabet`] inhabitants.**
//!
//! Each withholds exactly one of the answers the shift guard's conjuncts rest
//! on, so the certificate normal form's defensive checks become falsifiable.
//!
...
//!
//! [`CellAlphabet::position_order`]: gandr_theory_cell_complexes::CellAlphabet::position_order
//! [`CellAlphabet::convexity_discharge`]: gandr_theory_cell_complexes::CellAlphabet::convexity_discharge
...
/// [`CellAlphabet::Orientation`] enters [`gandr_theory_cell_complexes::Cell`]'s
...
/// precise defect `gandr_theory_deep_inference::normal_form`'s canonical
...
/// and `gandr_theory_deep_inference::normal_form::prim_address`'s `-
/// intension:` says in as many words that distinct inputs may in principle
/// collide. So a degenerate-but-legal tag hash is the honest triggering input
/// for
...
/// `gandr_theory_deep_inference::normal_form::prim_address` taken over it —
/// agrees with the toy suite's.
#[must_use]
#[inline]
pub fn lying_cell<L>(
    lhs: Toy,
