...
- `[[pre-start]]`: `copy-ignored` (fail-open ignored-state reflink) and `beads-pull` (`bd dolt pull || true` — cross-machine freshness of the shared database; worktree-to-worktree visibility needs no pull).
  Caches are copied deliberately because warming them is the step's purpose; `.rumdl_cache/`, `target/`, and `node_modules/` are therefore copied.
  Staleness is not an exclusion reason: excluding a misreporting cache hides its defect instead of fixing it.
  Exclusions exist only for cross-session state a fresh worktree must not inherit (`.beads/`, `PROMPT.md`, `SESSION.md`) and artifacts that break when copied (`coverage/`, `fuzz/artifacts/`).
  Each excluded directory is listed in **both** the `<dir>/` and `<dir>/**` forms, because the two match disjoint halves of Git's ignored-entry enumeration and a single form silently stops matching when unrelated `.gitignore` structure changes; the derivation and measurements are in the comment above `[step.copy-ignored]` in `.config/wt.toml`.
...
`PROMPT.md` and `SESSION.md` at the repository root are per-session coordination artifacts: the former carries a session brief and the latter live state.
Both are gitignored, never committed, and excluded from `copy-ignored` so a fresh worktree cannot inherit another session's brief or state.
A session that finds either file missing from `.gitignore` in any repository adds the ignore entry first.
