# Workflow: Agda design doctrine

> Read when: defining any structure, operation, or proof in `metatheory/` — representation, characterization, reasoning style, namespacing, and the boundary telescope.
> The lane's workflow — flags, gates, dependencies, solvers, opacity, the done-rule — is [agda.md](agda.md).
> These are tree-wide rules no single module header can own; the per-structure mathematical plan stays in the module headers.
