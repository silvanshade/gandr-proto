...
- **Recursive owned data is flat and id-addressed; recursion never routes through an owning pointer.** A recursive type's children are indices into an arena or interning table — never nodes linked through `Box`, `Rc`, `Arc`, or any other owning pointer, since pointer-linked recursion defeats destruction and duplication totality, threading conformance, and content identity at once.
  The general class is any `Deref`-owning type whose clone or drop recurses through the pointee; the named three are the floor a reviewer checks without analysis.
  Outside recursive positions: `Rc` is not used at all; `Arc` is the one permitted shared-ownership pointer, for whole values at boundaries; `Box` remains ordinary owned indirection.
  The pointer-linked recursive carriers still present in the core tier are retirement debt, not precedent: their retirement is tracked as `gandr-b9qv`, and new code does not extend them.
  **This rule is self-correcting on contact.** An agent whose change reads, produces, or passes a value carried by a pointer-linked recursive type first asks whether the touched seam can move to the flat carrier — or, for shared ownership of a whole value, to non-recursive `Arc` — within scope, and does so when the move is contained.
  When the move would grow the change disproportionately, the agent leaves the seam as found, flags the site on the retirement bead, and consults the advisor or owner before widening scope; silently extending a pointer-linked recursive surface is a review finding.
- **A hand-written relation is total over its own definition; delegating an arm is a claim.** When a custom comparison, ordering, or hash hand-writes some arms and delegates others to a derived or foreign relation (`==`, `cmp`, a sibling helper), the delegation asserts that the two relations agree on every position reachable through that arm — usually false the moment the property being relaxed reaches the delegated arm through any child, and invisible in review because a mostly-hand-written function reads as deliberate.
  Write every arm of the relation being defined, or state at the delegation site why agreement holds on everything reachable; a function whose arms implement two relations without that witness is a review-blocking finding.
  The obligation has a dual: a **deliberately coarse** arm — one that intentionally identifies more than its exact siblings — carries a witness that the looseness is intended and why, or a later reader repairs it into exactness and breaks the relation from the other side.
  Ported code is the hot spot: a port lands as one large diff reviewed as the port, so its relation definitions arrive unexamined.
...
- `# Adequacy`: `- hypothesis:` — a falsifiable claim naming which adequacy-ladder rung kills each decision surface's mutants, plus the distinguishing inputs and observations for the pointwise residue — then one `- witness:` bullet per witnessing test.
  Fixed grammar — the adequacy gates machine-extract it.
  **A witness resolves within its own crate's targets**, library and integration alike ([mutation-adequacy.md](mutation-adequacy.md) §"From contract to tests — how to read the blocks"), so a `- witness:` bullet names a test the crate's own suite runs.
  A test in another crate exercising this item is worth naming and is not a witness: put it in the item's prose, because an item whose only adequacy evidence lives downstream has not shown that its own suite separates its decision surfaces.
