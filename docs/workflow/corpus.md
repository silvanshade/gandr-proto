...
- **The `gandr-pro` skill has not crossed the reboot** — no live skill file exists; its carried home when ported is `crates/surface-corpus/skills/gandr-pro/SKILL.md`, and it resumes its maintenance train (mental model, surface tables, semantic traps, feature → model-example map) then.
- **Focused gate.** `cargo test -p gandr-surface-corpus` after any corpus change; the parser's zero-obligation count lock (`gandr-surface-parser` `acceptance::corpus_molds_to_zero_obligations`) guards the example trees — the tree-sitter highlight-parity suite is parked to rung F6 of the front-end port staging and guards nothing yet.
