...
> **Terminology (owner decision, 2026-07-19): the directed former is `Flow`, renaming ADR-79's `Step`.** The iu development now uses `Step*` for the free-involutive-word-category-adjacent machinery — the positive snoc kit this study leans on throughout — so the former's old spelling collided with the path-shape vocabulary; _flow_ is also evocative of directed topology.
> This study writes `Flow` everywhere; `Step` appears only when quoting ADR-79's original spelling, and `Step*` keeps its iu meaning.

The pre-reboot corpus fixes the directed **surface vocabulary** (ADR-79: the former — spelled `Step` there, `Flow` here — deliberately has **no `back`**: "the asymmetry of the directed family is visible in the vocabulary itself"; transformation notation `A ~~> B`; and the statement's outer shape already spelled, `Flow Type A B <~> (A ~~> B)` in the renamed vocabulary, outer connective groupoidal), the **stage gate** (ADR-76 D5: directed identity of codes gated on the levitation stage; ADR-79's honesty gate: the directed former enters the surface only when its rules land), and the **representation home** (the reflected directed ladder surveyed in §7, whose own module records directed univalence as `homSet(A,B) ≅ (A ⇒ B)`, _direction only, not scoped_, with two named prerequisites — §7.2 item 5).
...
| --------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| former          | `Path A x y`, endpoints invariant          | `Flow A x y` (ADR-79 D2, spelled `Step` there), endpoints invariant **at B7** — the contravariant-source/covariant-target reading (§7's `DirectedHom` slots) becomes load-bearing only at the reflected layer |
| intro           | `here(v) : Path A v v`                     | the diagonal intro `Flow A v v` (spelling open — §10 Q2); forced by the hom-with-J shape, it is the pre-reboot `DirectedHom::refl` (wyrd@failed-refactor:crates/gandr-vdc/src/directed/hom.rs:75-85)          |
...
**Coexistence mechanics**: `Path` and `Flow` land as **independent primitive formers**.
No kernel coercion `Path A x y → Flow A x y` lands at B7: §3's localization relation is O4's territory — a _theorem about realizations at the code stratum_ — and a kernel-level bridge would smuggle the core-coincidence statement in as an axiom, exactly the posture ADR-76 forbids ("never an axiom").
...
2. **Q2 — directed spellings.** The former is settled — `Flow` (owner rename of ADR-79's `Step`, no `back`) — and ADR-79 fixes the notations (`<~>`, `~~>`); still unnamed: the directed eliminator and diagonal-intro spellings, the `~~>` former's prose name (ADR-79's own candidate: `Transform`), and whether directed composition shares `then`.
...
3. **Q3 — a kernel `Path → Flow` bridge.** Recommendation on file (§6.2): none at B7 — the groupoid-to-directed comparison is O4's _theorem_, and a kernel coercion would assume it as an axiom.
