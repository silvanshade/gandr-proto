...
7. **Sealing is the first language-level unfolding barrier the normalizer meets** (§5, `HOLE-SEAL`; b3 §7 N3).
   ...
   Opacity here is **total, not scoped** — the GSACB scoped-unfolding line [L-6] stays the reserved **theory** layer for the B6+ dependent-core era (the wyrd-era "evidence layer", superseded by the reboot backbone; no backbone commitment to scoped unfolding exists), and B3's sealing is designed not to preempt it.
...
| **E** — two-layer unfolding control                                       | **engine** layer (Lean heights + transparency, no theory footprint) + **surface/theory** layer (GSACB [L-6] scoped unfolding, reserved for what wyrd called the evidence layer — in the reboot: the B6+ dependent core) | the unfolding discipline (§4.4); sealing lands in the engine layer, scoped unfolding stays reserved |
