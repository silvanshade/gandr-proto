...
| —                                 | `core-incremental`               | reboot-native, no wyrd source; schema-derived (not in the original table). Extracted from the duplicate item-granular checkpoint engines that had grown in `core-checker` and `surface-engine`                             |
