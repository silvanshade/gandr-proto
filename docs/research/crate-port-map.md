...
| —                                 | `core-incrementality`            | reboot-native, no wyrd source; schema-derived (not in the original table). Extracted from the duplicate item-granular checkpoint engines that had grown in `core-checker` and `surface-engine`                             |
...
| —                                 | `theory-circuit-algebras`        | reboot-native, no wyrd source; minted at the circuit-terms ruled boundary — schema-derived                                                                                                                                 |
| `gandr-vdc`                       | `theory-virtual-doctrines`       | built                                                                                                                                                                                                                      |
| —                                 | `theory-stone-duality`           | planned (original table), no wyrd source                                                                                                                                                                                   |
| `gandr-shell`                     | `runtime-host`                   | built (B1); **diverges** from the suggested `runtime-effects`; the source-runner faces did NOT re-wire here — the F3 host-signature ruling put them in `surface-engine`, and the reverse edge is a normal-dependency cycle |
...
| `gandr` (driver)                  | `surface-driver`                 | un-stubbed at front-end F5 (2026-08-08): script-runner face only; REPL/tui/lsp/mcp/fmt/build deferred                                                                                                                      |
