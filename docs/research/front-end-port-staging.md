...
* **`mise.toml` `grammar:test` is commented out** (`mise.toml:83-89`), with the note that the tree-sitter grammar package "is not in the repo yet (it returns with the surface-grammar port); restore this task — and its `gate:merge` entry — with it."
  The `agda:deps` task (`mise.toml:899-903`) runs `cargo run -q -p gandr -- scripts/agda-deps.gandr` — currently inert because the driver is a stub (§7-B ties the driver un-stub to re-enabling Agda vendoring).
...
| `gandr-shell`                     | host-effect runtime           | `gandr-runtime-host`             | `crates/runtime-host`             | **PARTIAL** — `run_program` + host; `run_source`/`run_source_file` parked |
| `gandr` (driver)                  | toolchain driver              | `gandr`                          | `crates/surface-driver`           | **STUB**                                                                  |
...
| `gandr-syntax`                    | flat-arena CST                | —                                | —                                 | **ABSENT** (cut #2)                                                       |
| `gandr-render-proto`              | render wire types             | —                                | —                                 | **ABSENT** (cut #1)                                                       |
| `gandr-grammar`                   | PBG + mold                    | —                                | —                                 | **ABSENT** (cut #3)                                                       |
| `gandr-parser`                    | melder push-machine           | —                                | —                                 | **ABSENT** (cut #4)                                                       |
| `gandr-pipeline`                  | lowering hub                  | —                                | —                                 | **ABSENT** (cut #5)                                                       |
| `gandr-corpus`                    | executable corpus             | —                                | —                                 | **ABSENT** (cut #6; fixtures pre-lowered into `core-sequent`)             |
...
* **`runtime-host` IS `gandr-shell` minus its lowering-coupled faces.** `crates/runtime-host/src/driver.rs` exposes `run_program(comp: &Comp) -> ShellOutcome` (`:104`), and its module doc states plainly: "`run_source`/`run_source_file` — the CST → core lowering convenience that ran the pipeline lowerer before handing the program to the host loop — [parked]" (`:29-30`), with continuation-key installation "parked with the surface engine" (`:96`).
  So `gandr-shell` is not a fresh port; it is an **adapt** — re-wire the two source-runner faces onto `surface-pipeline` once it lands.
...
* **`runtime-host`** — re-wire `run_source`/`run_source_file` onto `surface-pipeline` (add the pipeline dep the reboot re-cut removed), restoring the source-runner faces.
* **`surface-driver`** — un-stub the script-runner face (`gandr <file>`, the `run_source_file` seam), rewriting the stale wyrd dep names/paths; re-enables `mise agda:deps`.
...
| `runtime-host` (adapt)     | **adapt**              | add the `surface-pipeline` dep and re-wire `run_source`/`run_source_file`; reconcile the host-signature split (pipeline `host.rs` `HOST_MODULES` table vs `runtime-host` `sig`) so there is one authority, not two drifting copies                                                                                                              |
...
| **F5**            | `runtime-host` re-wire (`run_source`/`run_source_file`) + `surface-driver` un-stub (script-runner face; rewrite stale deps)                                                  | F3 (F4 for the corpus harness)                                                                       | driver smoke; `mise agda:deps` runs `scripts/agda-deps.gandr`; `14-agda-deps-walkthrough` reconverges                                                                           | M    | enables Agda vendoring + C4 re-sync |
