...
* **Refusing an import because the theory lacks a property the representation has, or the reverse.** This is the direction the corpus states least well and pays for most. gandr's theory is symmetric, so a **planar** theorem looks barred — but the merger already _is_ a planar tensor, the base being the free monoid on the colours, and canonicalization is the passage to multisets that makes the convolution symmetric.
  ...
  The **crossing-free-embedding** sense is the one the spider theorem is actually proved in: a connected diagram drawn in the plane with no wire crossing another, which is what makes its bead count the number of bounded connected components of the drawing's complement [@majid-rietsch-2021-planar-spider, cor 2.4].
  It is a fact about the **wiring inside a component**, and neither sense implies the other.
  **gandr has the first everywhere and fails the second at the diagram layer — measured, not argued.** `Gandr.Shape.Planarity` builds `k33 : Shape ⊤ [] []`, connected and wheel-free, whose derived incidence realizes the complete bipartite graph on three and three, so its underlying graph is non-planar [@kuratowski-1930-courbes-gauches].
  What that refutes is the bridge, not the instance: reordering the boundary — the outermost-permutation normal form `Rigid`'s recipe already runs — absorbs a boundary crossing and does nothing to the interior, so consuming a crossing-free-embedding theorem needs the diagram's own wiring to embed, and that is now [[metatheory/roadmap#meta-obligation-02|meta-obligation-02]], owed rather than had.
  **The failure is at the diagram layer and not at the cell layer**, which is why it did not surface sooner and why it does not disturb the rule: a `Cell` is connected and acyclic, hence a tree, hence planar, and it is **grafting** that leaves the fragment — the carrier's own `bigon` is two cells grafted into a composite that reconverges.
  The carrier says as much elsewhere in this document, at [[#The substrate is the full circuit-algebra rung]]: it is a circuit algebra in the Bar-Natan–Dancso sense, which is _defined_ by non-planar wiring diagrams.
  The same shape holds for **skeletality**: the coherence theorem for categorified cyclic operads is stated and rewritten **non-skeletally**, and discharged by transporting it through an ordering carried **as data** — the third of its three staged reductions builds a skeletal non-symmetric categorified operad whose objects are an operation, a distinguished entry, and a bijection totally ordering the remaining entries, and the theorem then follows from the coherence already proved for _that_ skeletal target [@obradovic-2017-thesis, sec. 4.1.5–4.1.6] [@dosen-petric-2015-weak-cat-operads] (the same construction and proof are in the standalone joint paper [@curien-obradovic-2019-categorified-cyclic-operads, sec. 2.5–2.6]).
  So the coherence is supplied by the **ordered** side and consumed by the symmetric one — the section discipline, worked, at an adjacent rung, with the thesis naming its price in the same breath: non-skeletally a symmetric-group action can always be pushed _inward_ onto the operands, skeletally that distribution does not work in general, and the source declines to claim anything about the opposite orientation — which is the one gandr's own recipe takes, permutations outermost [@obradovic-2017-thesis, app.
  A.2].
  A **different** claim of that thesis must not be read as this one: skeletal _exchangeable-output_ coherence is stated to follow by lifting the non-skeletal/skeletal equivalence, and the lifting is omitted in the source's own words — what the appendix works out in detail is the uncategorified equivalence, and that one is for symmetric operads [@obradovic-2017-thesis, sec. 4.2.2, app.
  A.1].
...
The signature unification landed the **sort index** on this universe (as built: `SignDesc.sorts`, `Code::Var` carrying its target sort, `CtorDesc.result` the real result-sort slot, all in `gandr-theory-levitation`): a multi-sorted signature is one description over a declared sort set — the recursive-occurrence code names its sort, every constructor targets one, and the surface's per-sort view is presentation only, never a signature stratum.
The index is genuine — a sort is never itself a description, and only finite declarations are importable — which is exactly what keeps code equality decidable.
...
Codata descriptions are the same codes under the final-coalgebra decoder, polarity-sorted from birth — and this is now a cited theorem, not an ergonomic hope: the ν decoder is the **dual assembly** of the μ decoder, the same signature read through the opposite dialgebra orientation, turning coproducts into products and demanding dependent products the μ decoder never needs [@basold-2015-fibrational-dialgebras] [@basold-geuvers-2016-dependent]; intensionally this yields only weak final coalgebras, and the no-η codata stance _entails_ that weakness rather than merely tolerating it [@basold-2018-mixed].
The bill of that weakness, priced so no later consumer discovers it by surprise: decoder **functoriality** is lost along with uniqueness of unfold [@basold-2018-mixed]; part of the toolbox this section would like to import consumes **strong** finality — the stream-processor representation theorems need unfold's uniqueness, and the quotient that would restore it is not decidable and possibly hyperarithmetic [@hancock-pattinson-ghani-2009-stream-processors]; the fibrational **coinduction proof principle** is defined by preservation of terminal objects, so it is not even statable at weak finality and the ν side owes quotient structure gandr's codata stratum lacks — the Löb-style recursive-proof rule is the repair route, unconditionally sound with no final coalgebra at all [@basold-2018-recursive-coinduction] [@ghani-johann-fumex-2013-indexed-coinduction]; and a guard's nesting depth is **observable** intensionally, so quotienting it away breaks well-foundedness, with the index escape the working mitigation [@kidney-wu-2025-graph-coinduction].
...
The signature unification paid the first installment of that index change — the sort index above, with the constructor layer's container agreement made checkable (`CtorDesc::arity` exhibits each constructor as a single-output bridge arity over the sort set) — while the multiset-of-output-sorts generalization remains the multi-output lane's.
...
**Cells at every dimension.** The surface names sorts (0-cells), constructors and operations (1-cells), named directed rewrites (2-cells), and declared coherences between rewrite composites (3-cells), in the ruled sign block's judgment-style member spellings: `sort S : Type`, `data C : sig`, `oper f : sig`, and `rule name : lhs ==> rhs` — with dimension read off the endpoints rather than off a new arrow, so a rule between rule-sorted endpoints _is_ a 3-cell with no new syntax, and the `cell` keyword stays reserved for the dimension ≥ 4 tower ([[surface-language/circuit-cells#The block form, ruled]] owns the spellings; [[surface-language/higher-cells]] the higher-dimensional design).
...
Carrying it in `Match` would put a number inside every listing-algebra lemma — `match-insert`, `match-cap`, `cap-swap`, `match-cap-insert` — none of which can close a circle, while at the code exactly the operations that close one touch it.
...
That decline is **provisional in its source and now carries the reversal condition it crossed without** — it is revisited when the split stops discharging the adjoined cells in one theorem, which is what the codiagonal's class would do ([[metatheory/guards#Clones as the arena's morphism class]]).
...
The trigger is architectural and single: _materialize a coherence witness, then have a kernel check it_. gandr's exposure is therefore one named condition — a coherence witness becoming an Agda term in the gate root — and the composition law of the wiring calculus is the instance that has now measured it: **both halves are closed**, so `match-comp-assoc` holds with no hypothesis and no parameter, and the deepest coherence anywhere on its route is five layers rather than the four the estimate had stopped at ([[metatheory/roadmap]]).
...
**The lax level's derivation is unconditional exactly where this section leans on it, and conditional one step further out, so the scope is carried rather than assumed.** The coercion is the lax-monoidal structure of the cobordism double category [@mellies-stefanesco-2020-csl, thm 4.4], which carries no hypothesis; the same inequality for the _generalized_ composition holds only under a filling hypothesis the source verifies for its code templates and states explicitly may fail for the template of separated states ([[implementation/template-games#The proof-scope of the derived Hoare inequality]]).
...
The measured finer alternative is declined knowingly, not unknowingly: the asynchronous-games treatment quotients reschedulings by "same induced bijection on step indices" [@mellies-2021-template-games] (the same device as the residual line's ancestor function on redex indices — one construction in both programmes), and gandr's identity is strictly coarser on two axes, forgetting the induced permutation and never comparing the two paths; the same programme makes a deliberate at-most-one-2-cell ("locally posetal") choice for one of its own bicategories, the freely generated lock-graph bicategory with at most one invertible 2-cell between any pair of morphisms [@mellies-stefanesco-2020-csl, sec 8], so the coarse discipline has a named precedent as a design choice — not a theorem.
A returning derivation has no replayable representative, and the derivation dimension of the wheel axis is ruled declined with a recorded reversal condition ([[implementation/circuit-terms#Wheels, and which structure the cell layer takes]]) — replay-equivalence never compares returning paths by ruling rather than by omission, and a fixpoint the completion loop encounters reports through the budget path instead of becoming a cyclic certificate.
...
> **The hazard is that disjointness stops implying independence once matches must be convex.** In convex diagrammatic rewriting, two rule applications on **disjoint** sets of hyperedges can block one another, because applying one creates a directed path that destroys the other's convexity [@bonchi-gadducci-kissinger-sobocinski-zanasi-2022-string-diagram-rewriting-ii, ex 45]. gandr's shift quotient is TCB-adjacent — it is what a `cells_equal` normal-form fast path would decide, though as built `cells_equal` decides boundary equality plus two replays and carries no such fast path yet — so at the circuit rung it is warranted only on a fragment where a match cannot be made non-convex, and the guard is now decided rather than owed ([[implementation/circuit-terms#circuit-terms-spike-07]]).
> Today's cell store is term-shaped and the hazard is excluded there — proved by the strong connectedness the cut grammar forces, not merely observed; it becomes live at exactly the moment the cell alphabet changes, which is why the guard was decided before the change rather than after.
