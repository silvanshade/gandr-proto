...
Under the sized-types rung this stops being a pun and becomes literally true: `f[<]` is instantiation of an erased size argument, at a discipline the sized design pass fixes (the common case a fresh size strictly below the ambient one; see the reopened clauses below for why the strict-descent reading is no longer fixed in advance).
...
| **sized**       | both sigils elaborate to erased size applications; the explicit `size =` form opens in the same slot; the strict-descent and conversion-quotient sub-clauses are design-pass-owned (reopened below)                                          | the sized-types design pass, including the bounded-quantification and infinite-size interaction, the subtyping-versus-conversion-quotient choice, and the constant-size well-founded-measure carve-out | **ratified as the destination** (owner ruling, 2026-08-02, on the mixed induction-coinduction sweep) and still gated on that design pass   |
...
The mixed induction-coinduction sweep confirmed this caveat at theorem grade twice over — syntactically and semantically — and confirmed the direction split as presentational: one typing rule covers every marked occurrence and never consults direction [@abel-pientka-2013-wellfounded-copatterns].
...
Two clauses this section once fixed alongside that caveat did **not** survive the same sweep, and are **reopened as design-pass-owned** rather than carried as normative (owner ruling, 2026-08-02): the conversion-quotient clause — the swept sources realize size irrelevance by **polarized size subtyping plus compile-time erasure**, a directed discipline, not a symmetric conversion quotient — and the strict-descent clause, contradicted by the well-founded-cofixpoint route, whose inductive side descends a user-supplied well-founded measure at **constant** size, with its motivating examples holding a stream at the infinite size.
The sized design pass decides both; nothing before it may cite either clause as settled.

The ladder's ordering is also a soundness hedge, and the hedge is now sourced.
Agda's shipped implementation admits proofs of the empty type through sized types, and all three demonstrations remain open at the tracker: "Equality is incompatible with sized types" (<https://github.com/agda/agda/issues/2820>, open since October 2017), "Well-founded induction on Size is inconsistent" (<https://github.com/agda/agda/issues/3026>, open since April 2018 — the instance that matters most here, since it is exactly the sizes-with-accessibility combination the design pass contemplates), and "Proof of ⊥ with sized types" (<https://github.com/agda/agda/issues/6002>, open since July 2022); the survey collecting the diagnosis is [@chan-2021-sized-types-state].
The determination of record: **the defect is the implementation's, not the theory's.** The shipped infinite size satisfies `∞ < ∞`, so the size order the checker trusts is circular and well-founded induction on sizes collapses; the sound formulation gates reduction under bounded size abstraction on a size-context consistency check, and it is exactly the omission of that gate that the naive combinator pays with strong normalization [@abel-pientka-2013-wellfounded-copatterns]. gandr's sized rung adopts the consistency-gated formulation from the outset, which is why the ratified direction survives the implementation history that motivated the hedge.
