...
* Status: **partially ruled.** The block form's core spellings — the declaration shape, the arrow grid, the statement forms, and the port discipline — are **ruled** (owner, 2026-08-02) and recorded at [[#The block form, ruled]]; the sections below that sketch alternatives to them are superseded where they conflict and are annotated in place.
  Everything else remains **design sketch**, and no example has a corpus witness or an elaboration; the ruling chose spellings, not semantics.
  One piece of the semantics is landed beside them: the derived-boundary route of [[#The derived pair meets the sphere by checking, not by synthesis]] is specified and checked at the declaration table, and the congruence block's boundary pair is exercised by crate fixtures — the only example with a checked semantics, and only for its boundaries, because the block form's surface is not yet wired to it.
...
As built, the wiring and the two readings are `gandr-theory-levitation`'s `circuit` module (`derive_boundaries`), the sphere and the wiring sit together in the declaration table's `DataDesc::circuits`, and the comparison runs in `check_desc`, surfacing `WfKind::DerivedBoundaryMismatch` for a disagreement and `WfKind::CyclicCircuitWiring` for a wiring that derives nothing.
...
* **This document's status is the load-bearing part of it, and it split on 2026-08-02.** The block form's core spellings are ruled ([[#The block form, ruled]], owner, in design dialogue): those code blocks are spellings.
  No example has a corpus witness or a surface elaboration, and the derived-boundary route is the one piece carrying a checked semantics — specified at [[#The derived pair meets the sphere by checking, not by synthesis]] and exercised by crate fixtures — so everything else, elaboration and holes and aggregation included, remains a sketch and its code blocks remain shapes.
