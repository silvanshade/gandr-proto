...
* Status: **partially ruled.** The block form's core spellings — the declaration shape, the arrow grid, the statement forms, and the port discipline — are **ruled** (owner, 2026-08-02) and recorded at [[#The block form, ruled]]; the sections below that sketch alternatives to them are superseded where they conflict and are annotated in place.
  ...
  One piece of the semantics is landed beside them: the derived-boundary route of [[#The derived pair meets the sphere by checking, not by synthesis]] is specified and checked at the declaration table, exercised by crate fixtures rather than by a corpus program, because the block form's surface is not yet wired to it.
...
**The direction is load-bearing rather than a convention.** A sphere read off its own filler makes every filler parallel to its own boundary by construction, so the judgmental globularity the sphere device buys would be vacuous — and the mis-gluing it is there to catch would resurface downstream, at the site the sphere exists to keep it away from.

**Computing the pair is one resolution of the declared output port, run twice.** Each body statement binds exactly one output port, so a body is a map from port names to what produces them, and a boundary term is what the declared output port unfolds to under one of the two readings:
...
| bound by a redex line, under the source reading | the applied rewrite's **source** boundary                            |
| bound by a redex line, under the target reading | the applied rewrite's **target** boundary                            |
| bound by a redex line **to itself**             | itself — the opaque endpoint an unpinned rewrite-sorted port carries |
...
Any _other_ way a port reaches itself is a wiring cycle, no boundary term unfolds from it, and the derivation refuses it naming the port — which is the derivation's own termination guard and **not** the back-edge sweep: the sweep's spelling correction, _this cycle has no `feed`_, is still owed at [[#The block form, ruled]].
...
The feature is not user-writable at this rung: the wiring is built through the crate's own constructors, and the surface route arrives with the block form's lowering.

**One discipline does not transfer, and saying so is what lets the congruence case land at all.** A written rule's pattern variables are bound by its left-hand side ([[higher-cells#Named 2-cells and the identity discipline]]), so its right-hand side may introduce none of its own.
...
The fresh-right-hand-side-variable rule is therefore not run against a circuit rule's sphere — running it would refuse `cong2` — and the binder that does bind those names is the port list, checked by the name-set fold.

**What the declaration fixes today, stated as the limit of the route rather than left to be discovered.** The ruled port-list signature fixes the sphere's sorts and its interface; the endpoint _terms_ come from the parameter telescope, and the pinned binder form `rule p : x ==> x′` is what supplies them — the recorded easing on this question, now cashed as the checking route.
Where every rewrite-sorted binder is unpinned, the declaration fixes the sphere only up to its sorts, and the comparison bites on exactly as much as the declaration fixed; the explicit boundary spelling carried at [[#Open questions, dispositioned]] is the other way to fix it.
...
**Scope, so the next rung inherits it rather than rediscovers it.** The derivation reads **one** declared output port, because the boundaries it produces land on the term-shaped store and a term has one root; a many-out interface is the cell-alphabet question, and this route does not admit it by omission.
