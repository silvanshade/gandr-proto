...
> **The generality ruling (owner, binding).** The substrate carries the full generality of circuit algebras — many-in/many-out arity, wheels, disconnection, and the cut — and gandr's restrictions are enforced by **static analysis, not structure**.
...
The corpus witness inventory names the wheel guard, the acyclicity decline, and the symmetry refusal as owing exactly such witnesses.
**Surfacing these three axes is what makes those witnesses writable.** That, and not expressive power, is the argument.
...
This is the feedback-category structure — a symmetric monoidal category with a monoidal endofunctor `delay` and a `feedback` operator taking `f : x ⊗ delay(m) → y ⊗ m` to `f : x → y` — as implemented in DisCoPy, where the delay is carried **on the object** as a time step and `wait` is definable as the feedback of a swap [@discopy]. (DisCoPy attributes the definition to Di Lavore et al., 2022; that paper is **unread here and its locator is owed** — nothing below rests on it beyond the implementation this document did read.)

The decisive fact for gandr is the relationship between the two structures:
...
That places the corpus's own honesty note exactly.
A cartesian trace **is** a fixpoint [@hasegawa-1997-recursion-cyclic-sharing], which is why wheels are fenced to the guarded form — and the fence is not a convention laid over a traced structure, it is the statement that **gandr wants the feedback structure and not the traced one**.
The `fix`-versus-wheel firewall is the static-analysis face of exactly that difference.
...
A diagram with a hole is a context, and the optimizer's overlap shapes are exactly hole-contexts.
...
The related precedent, read: in DisCoPy a spider is **boxless** — `spiders(n_legs_in, n_legs_out, typ)` introduces no generator at all, it simply repeats one wire label on both sides, so many-in/many-out is "the freedom of the wiring map to be non-injective" rather than a primitive [@discopy].
...
| 7   | whether the reserved `↻u` marker is consumed by this design                                                                         | **carried**; it is currently deferred-with-reasons and unspelled ([[roadmap]])                                                                                                                                                                                                                                                                                                      |
...
| 11  | the Di Lavore et al. feedback-category paper                                                                                        | **locator-pending**: read only through DisCoPy's citation of it. No claim here rests on it beyond the implementation actually read                                                                                                                                                                                                                                                  |
...
* The **feedback-versus-trace distinction** is the one substantive design finding this pass added, and it rests on an implementation that was read directly plus a paper that was not.
  The dependency is marked at the claim.
