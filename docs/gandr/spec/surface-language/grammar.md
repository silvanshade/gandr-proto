...
1. **Operator Form** — no concatenation may expose two sort holes adjacent, even through nullable segments: **every sort hole is separated by at least one tile.** Mandatory call/ctor parentheses and keyword-led forms satisfy this structurally; a juxtaposition-application _form_ and an open-mixfix _form_ violate it — the structural reason those forms cannot be folded.
   The gate constrains **forms**, not adjacency at parse time: the shell sub-grammar reaches juxtaposition without a form, since each shell word is its own single-tile `Expression` atom and the melder joins the juxtaposed atoms, with grouping left to the semantic stage (`crates/surface-grammar/src/surface/type_shell.rs`, `add_shell_rules`).
   So the gate does not by itself settle whether gandr could have juxtaposition application; the rejection that does is doctrinal and economic ([[recursion#Application syntax, rejected]]).
...
| W4d/W4e growth                     | `data`, `codata`, `for`, `while`, `loop`, `break`, `continue`, `import`, `module`, `in`, `rec`, `op`, `rule`, `with` (globally reserved); `infix`, `infixl`, `infixr`, `prefix`, `postfix` (contextual — fixity tiles only, admitted where the `op`-led fixity form expects them) |
...
| form                           | shape                                                               | status                                                                                                                                    |
| ------------------------------ | ------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| operation member               | `oper name(params) -> R` inside `data`                              | parse-and-decline; the retired `op` lead also parses here, and the stage-0 elaborator declines it with the respelling                     |
| multi-output result            | `oper name(..) -> (o1: A, o2: B)`                                   | parse-and-decline; the named-tuple result is **local to the `oper` member**, never a general type form                                    |
