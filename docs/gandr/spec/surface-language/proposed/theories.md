...
* **The declaration substrate is ordered, incremental, and rollback-carrying.** A source file is a flat ordered list of top-level items ([[../../surface-language#Items: what a file is]]); the incremental pipeline checkpoints each item with a dependency footprint, revalidates rather than blindly reuses, and maintains item identity across edits by splicing an order-maintenance structure — `gandr-core-incrementality`'s `checkpoint`/`footprint`/`region` modules, `gandr-surface-engine`'s `edit` module (twelve localized actions, including insert and delete on the item list), and `gandr-theory-orders` entire ([[../../implementation/incremental-pipeline#Granularity: the item and the node]]).
...
The source's dedicated introduction form for refined theories — a `make` constructor from the residual telescope [@sterling-2026-pterodactyl-worklog, tree 01HP] — is thereby subsumed: instance introduction is ordinary record introduction, and no theory-specific constructor former is carried.
...
* **Selfification runs at the marked site, and it is what keeps coercions non-dependent.** Where a `cast` checks a theory value $M : T$ against a target, the resolver first forms the **maximal refinement** of $T$ over $M$ — every field pinned to its projection from $M$, a definitional singleton — and then resolves the canonical coercion from that refinement [@sterling-2026-pterodactyl-worklog, tree 01HS].
  A coercion whose target mentions the coerced value (a refinement `T / {car => M.car}`) is thereby an ordinary edge lookup from the singleton refinement, never a dependent coercion.
* **Identity edges are installed, not derived**: a theory's edge to itself is the identity, carried as data in the register.
* **Link edges allow local coherence declarations** — edges added _locally_, so the preorder need not be fixed in advance — the modular-type-classes mechanism [@dreyer-2007-modular-type-classes]; **cycles are tolerated**, a loop being a degenerate diamond the insert-time coherence check covers like any other [@sterling-2026-pterodactyl-worklog, tree 01HD].
