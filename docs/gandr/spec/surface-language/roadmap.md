...
| ---------------------------------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `op` operation members (incl. multi-out)                                     | parse-and-decline                        | the cell layer consumes operation frames (the engine's fusion rung); the multi-out result needs the term face of [[../implementation/circuit-terms]]                                      |
...
What still declines is the **lowering** route (`gandr-surface-engine`'s `lower/codata.rs`), which is the route an ordinary program takes, so `parse-and-decline` remains the honest status for the default pipeline; the multi-out result is declined on both routes.
