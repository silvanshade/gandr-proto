...
| --------------------------------------------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `oper` operation members, incl. multi-out (the retired `op` lead parses to be declined) | parse-and-decline                        | the cell layer consumes operation frames (the engine's fusion rung); the multi-out result needs the term face of [[../implementation/circuit-terms]]                                      |
| `rule lhs ==> rhs` members (the retired `~>` lead parses to be declined)                | parse-and-decline                        | the cell store accepts surface-declared cells (the L2/completion rung); rule **conditions** will grow the form — the condition-syntax seam is a binding forward constraint on the grammar |
...
| per-symbol attribute slots                                                              | parse-and-decline                        | the attribute semantic tier ([[attributes#attr-decision-03]])                                                                                                                             |
...
**The first two rows are route-wise, not tree-wise, and the distinction is load-bearing.** Their graduation conditions have already fired on the **description** route: `gandr-surface-engine`'s `desc_cells` pass drives `gandr-theory-computads`' `elaborate_data_desc`, so an `oper` member's arity and a `rule` face both become cells in a store, with runnable model witnesses at `crates/surface-corpus/examples/model/desc/desc-op-cells.gandr` (two rules, four store cells) and `desc-cell-face.gandr`, and the many-out decline pinned at `examples/pathological/desc/many-out-op-declined.gandr`.
What still declines is the **lowering** route (`gandr-surface-engine`'s `lower/data.rs::collect_data` for a `data` block, `lower/codata.rs::collect_codata` for a `codata` one), which is the route an ordinary program takes, so `parse-and-decline` remains the honest status for the default pipeline; the multi-out result is declined on both routes.
Read the two statuses as the lowering route's, and read the description route's acceptance as the seam [[higher-cells#Discharge by completion]] builds on.
...
- **extern member attributes `@unwind`/`@variadic`/`@repr(c)`** — the bare-`@` form conflicts with the `@[…]` block discipline ([[attributes#attr-decision-01]]); these are semantic rather than inert, so they also want the tier that does not exist ([[attributes#attr-decision-03]]), and the extern-member attribute story is unfixed;
