...
**The third part of the series answers both halves at once and is the primary read**: it proves confluence for double-pushout rewriting with interfaces **decidable**, and derives effective procedures for deciding local confluence of symmetric monoidal theories **with and without Frobenius structure** by critical-pair analysis [@bonchi-gadducci-kissinger-sobocinski-zanasi-2022-string-diagram-rewriting-iii].
Its device for the non-Frobenius case is **path joinability**, which lifts finitely many joins of a critical pair to an arbitrary context in spite of the non-local constraints a generic symmetric monoidal theory imposes — and a critical-pair analysis is precisely what gandr's completion engine already runs.

### meta-spike-17

**Does decidable confluence transfer, and does the critical-pair procedure?** Confluence for double-pushout rewriting with interfaces is decidable, and the decision is delivered as an effective **critical-pair analysis** for symmetric monoidal theories with and without Frobenius [@bonchi-gadducci-kissinger-sobocinski-zanasi-2022-string-diagram-rewriting-iii] [@bonchi-gadducci-kissinger-sobocinski-zanasi-2017-confluence-interfaces]. gandr's completion engine ships with the honest caveat that _completed_ means the worklist drained and not that the slice is confluent, and three obstruction classes are silently dropped — and it already enumerates critical pairs, so the question is not whether to adopt a new algorithm but whether the one it has decides what this result decides.
