...
**Two halves of this spike stay open.** Whether the removal/rebuild pair is an instance of the context comonad is untouched, and the built canon is a canon on the **diagram view**, not on the carrier's construction terms — so `Rigid.canon-sound` below keeps its own recipe and its own residual, and whether the two canons agree is an owed question rather than an assumption.
