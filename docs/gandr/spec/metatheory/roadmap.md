...
* **`Rigid.canon-sound` at the circuit rung**: the construction-term normal form (permutations outermost, ordered tree monomials, unique minimal representative), with the **monomial-to-monomial rewriting condition** checked before anything leans on it.
  **A bounded hazard is now recorded against it and must be read before the obligation is attempted** (2026-08-01).
  A published coherence proof for categorified cyclic operads states that **non-skeletality is crucial for the rewriting it uses**: in a non-skeletal setting a symmetric-group action can always be pushed _inward_, from a composite onto its operands, by orienting the equivariance law that way, and its first reduction depends on this; in a skeletal setting that distribution "doesn't work in general", exhibited at a three-element composite, and the authors add that they do not know whether orienting the equivariance the **other** way would serve [@obradovic-2017-thesis, app.
  A.2].
  Three things bound the hazard rather than let it stand or fall as a refutation. gandr's ordered representation is skeletal, so the move that fails is available to it only in the direction that fails; gandr's recorded recipe pushes permutations **outward**, which is the direction the source declines to claim about in either direction; and skeletal and non-skeletal presentations are proved **equivalent** in the same work, with skeletal coherence proved by reduction to the non-skeletal statement, so what a skeletal presentation loses is a proof technique and not a theorem.
  Read into gandr's own vocabulary: this is "ordering is a section" holding, with a named price — an ordered representation is not coherence-neutral, and the specific rewriting move it forgoes is now on the record instead of being discovered at proof time.
