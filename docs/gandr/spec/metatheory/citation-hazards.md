...
- **The polygraph presheaf source is titled more narrowly than its strongest theorem** [@henry-2019-nonunital-polygraphs]: the title says _non-unital_, but Theorem 2.4.8 makes the **source-positive** polygraphs a good class of polygraphs, and Definition 1.3.2's non-unital (positive) class is the strictly stronger one Corollary 2.4.9 reaches separately, alongside the opetopic class.
  So citing the paper by its title for the condition a design satisfies over-states that condition — which is exactly how this corpus once recorded gandr's escape hatch as non-unitality, and how the false claim "polygraphs are a presheaf category _only_ when non-unital" got written down.
  The identity criterion the side condition is tested by is that source's section 1.2.3: an $n$-arrow is an identity exactly when no generator of dimension $n$ occurs in it.
  **A second, unrelated hazard sits one word away.** "Nonunital" also names gandr's circuit-algebra rung — nonunital wheeled props, the monad $T^times$, a nonunital Frobenius supply — which is a different notion in a different theory and shares nothing with the polygraph condition but the spelling; check which one a sentence means before citing either.
...
- **The general spider theorem is a _planar_ theorem, and planarity is what buys its generality.** The statement that a connected diagram equals a standard form with $m$ in, $n$ out and $j$ beads holds for **noncommutative** algebras and **asymmetric** Frobenius forms, and it is proved for planar connected diagrams [@majid-rietsch-2021-planar-spider, cor 2.4].
  ...
  **And "planar" is two conditions in this row, so say which one a citation is spending.** The **base-objects** sense is the merger's — the monoidal base is the free monoid on the colours, so interface objects are lists rather than multisets — and it is what canonicalization is a section of.
  The **crossing-free-embedding** sense is the theorem's own: corollary 2.4 is stated for a connected diagram drawn in the plane, its bead count being the number of bounded connected components of the drawing's **complement**, which is a condition on the wiring inside a component.
  **gandr's diagrams do not satisfy the second**, and this is measured rather than reasoned: `Gandr.Shape.Planarity` builds a connected, wheel-free `Shape` whose derived incidence realizes the complete bipartite graph on three and three, so its underlying graph is non-planar [@kuratowski-1930-courbes-gauches].
  Reordering the boundary absorbs a boundary crossing and leaves the interior alone, so the consumption this row licenses is the base-objects one; the crossing-free one is owed at [[roadmap#meta-obligation-02|meta-obligation-02]] and must not be taken as already had.
