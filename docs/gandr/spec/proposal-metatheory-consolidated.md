...
| **L3 — arity kits**            | the linear kit and the graph kit; one interface, two instances (M1)                                                                                                 | `Gandr.Arity.Path`, `Gandr.Shape.Graph`, `.Graft`                          | §5  |
...
| `Gandr.Shape.Graph`           | the graph arity kit: `Shape Γ Δ` indexed by its `List Ob` interfaces, listings primary and incidence derived; the connectivity predicates with their refuters; the block swap and the generic corolla family, a cell for every profile; decidable equality of shapes |
| `Gandr.Shape.Graft`           | the arity operation: grafting along the shared interface, over its listing algebra; the vertex listings concatenate; the derived unit is a two-sided unit                                                                                                            |
...
A third rule governs every _definition_ below, added after it cost three retrofits in one session: a definition that will be reasoned about must not `with` on its own recursive call, because that compiles to an auxiliary no later lemma can reach — write the recursive clause as an application instead.
...
| **S-7**  | **Extract the arity interface** and generalize the telescope over it (§5.5). The extraction **pass is run** against both instances and its findings are §5.4's; the **record** waits on the graph kit's heterogeneous comparison and grafting's inductive graph. This edge is valid only because S-3 produces an interface-**indexed** carrier: an unindexed one leaves the abstraction nothing to quantify over, and the dependency then fails silently | whether M1 is executable rather than a slogan                                                      | 2–3 days             |
