...
* [[surface-language/proposed/theories]] — **proposed, nothing built**: the theory layer — named telescopes with generative identity, retroactive definitional extension, and shallow refinement, reconciled with the module layer as two access disciplines over the same record values, with the species distinction (telescope extension vs signature growth), the coercion register, and the Model(S) correspondence.
