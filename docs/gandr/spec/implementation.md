...
Five checked structural invariants: reference integrity; scope (a focused command has no free covariables); focus (a producer in argument position is a substitutable value); arity (tag-declared counts, and **a destructor frame carries exactly one trailing return continuation — checker-enforced**, while constructors' empty consumer lists and the n-ary grammar on primitives are construction-site discipline and the reserved multi-output growth point of [[implementation/circuit-terms]]); polarity (per-form constraints, with strategy a per-cut orientation: positive fires the producer-side binder first, negative the consumer-side).
...
The bridge arity for multi-output operations is the bridge-diagram composite with the Π-layer/Σ-layer firewall from the metatheory track; the lane that consumes it, layer by layer, is [[implementation/circuit-terms]].
...
The (co)recursion surface's design and its full ladder are in [[surface-language/recursion]]; the core former that surface lowers into does not exist yet, and its design is [[implementation/proposed/recursion-former]].
...
* [[implementation/circuit-terms]] — computing with circuit-algebra terms: the four axes, the fan-in and supply vocabulary, the layer-by-layer substrate, the diagrammatic-rewriting findings, the design questions, the staged rungs, and the spikes.
* [[implementation/template-games]] — the template and cobordism apparatus gandr adopts now and the concurrent separation logic it defers: the tile pairing and the three axioms it gates on, polarized footprints and their guard fence, the environment polarity a template forces, the virtual-cobordism and store-transition-support constraints, and the six theorems the adoption owes.
* [[implementation/proposed/separation-logic]] — **proposed, nothing built**: the separation-logic half of that line, held against the heap and reference machinery gandr is committed to — what the import gives once the machinery lands, the requirements ledger that machinery must satisfy, what can be built ahead of it, and what stays cut regardless.
* [[implementation/foreign-interface]] — the foreign C-ABI boundary: a foreign call as an effect operation, loading as a capability, the library lifetime as a linear resource, abort-on-unwind, the boundary type mapping, the two execution paths, and what the boundary requires of the compilation backend.
* [[implementation/durable-computation]] — the durable, reversible, replicated-computation direction: what gandr may already carry for it, and the reference points the reversible-computing literature supplies.
* [[implementation/proposed/recursion-former]] — **proposed, nothing built**: the one core former user-level recursion needs — the fixpoint over a graded thunk, its checker and operational rules, its realization by machine re-entry rather than a heap cycle, why it is a former rather than a builtin, the memoization argument that keeps a divergent fixpoint uncached, the boundary against the linear feedback wheel, and the termination ladder past the step budget.
* [[implementation/proposed/packages]] — **proposed, only its manifest vocabulary built**: the package and build manager — the import surface and its lowering to module values, content-addressed identity with its cache and hydration, distribution over endpoint coordinates, typed ascription as the version-compatibility check, distributed builds, and the attribute-hosted manifest with its field-role taxonomy and content-address boundary.
