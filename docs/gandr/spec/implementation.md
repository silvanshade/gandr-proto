...
* [[implementation/template-games]] — the template and cobordism apparatus gandr adopts and the concurrent separation logic it declines: the tile pairing and the two axioms it gates on, polarized footprints and their guard fence, the environment polarity a template forces, the virtual-cobordism and store-transition-support constraints, and the six theorems the adoption owes.
