...
Five checked structural invariants: reference integrity; scope (a focused command has no free covariables); focus (a producer in argument position is a substitutable value); arity (tag-declared counts, and **a destructor frame carries exactly one trailing return continuation — checker-enforced**, while constructors' empty consumer lists and the n-ary grammar on primitives are construction-site discipline and the reserved multi-output growth point of [[implementation/multi-out]]); polarity (per-form constraints, with strategy a per-cut orientation: positive fires the producer-side binder first, negative the consumer-side).
...
The bridge arity for multi-output operations is the bridge-diagram composite with the Π-layer/Σ-layer firewall from the metatheory track; the lane that consumes it, layer by layer, is [[implementation/multi-out]].
...
* [[implementation/multi-out]] — the destination-passing term face: the layer-by-layer substrate, the diagrammatic-rewriting findings, the design questions, the staged rungs, and the spikes.
