...
| kernel-core skeleton               | the trusted S1 core: term language, declaration vocabulary, environment with one admission choke point, small conversion, export writer with reserved annotation slots                      | **closed**                                                                                         |
| modules                            | 1ML-style functors and first-class modules over CBPV as their own primitive layer                                                                                                           | in progress (primitive modules, sealing with export replay, generative functors, package Σ remain) |
...
| tail                               | arbitrary-precision numerics, stone duality, sessions/worlds, intersections/unions, codata, codecs, user-facing effects, cost-as-effect, value semantics/modes, the shell language, erasure | ordered, open                                                                                      |
...
| 3        | `core-sequent` (→ checker, kernel-core, strata, artifact, prolly-trees), `theory-levitation` (→ checker)                                                       |
...
**`kernel-core` is the certified S1 core**: pure polarized CBPV terms and types, definition/axiom vocabulary, an append-only environment whose single admission choke point re-derives everything (the elaborator gets no credence), and quarantined conversion.
...
No trusted memo table enters the TCB: the conversion early-out decides only reflexive pairs, and pointer-keyed kernel caches are a priced trust expansion deliberately not taken at S1.
...
The crate is intentionally **monomorphic** over this alphabet — no trait, no type parameter — which is what the alphabet-polymorphism spike in [[metatheory/roadmap]] exists to lift.
...
`theory-nominal-automata`: sort-tagged atoms and a monotone gensym with an atom-versus-variable sort boundary (what preserves unitary most-general unifiers); unforgeable atoms, deliberate non-`Copy`; **no automaton exists yet despite the name**.
...
The grammar is a checked **precedence-bounded grammar** over named precedence DAGs — hand-written constructor code, no codegen, in the tylr lineage (not BNF, PEG, tree-sitter, or GF); a mold is a zipper into the grammar interned to a compact id.
...
Vocabulary decisions of record: the universe keyword is `Type`, never `Set`; a small closed set of globally reserved keywords with fixity classes contextual; type operators are right-associative (an unparenthesized flat chain is a user-visible error); imports are `import "URI" as name` (plain string, file scheme now, others zero-grammar-change later); the shell fragment's braced parameters are deliberately distinct from string interpolation, with subshell brackets and file-descriptor redirections in the shell context and host-escape reserved; the bare binary on a terminal is the minimal shell-REPL and the programming environment is explicit.
Ten reserved-slot forms parse and decline until their semantics land — among them the multi-output operation, the directed rewrite member, grade-prefixed fields (numeral or ω only), parameterized observations, and generalized constructor result types.
...
The corpus is 103 executable files in three firewalled trees (model, pathological, and parse-only surface witnesses) with directive-comment modes; the driver binary is a stub until the crates it wraps land; the render-remote crate is the renderer firewall — leaf wire types shaped as the projection of a binary session type, parsing and typing nothing.
...
**There is no capability or sandbox model** — ambient always-resume posture, no path allowlist, no per-signature grant — and the linear-zone claim is vacuous with multi-shot resumption; stating this here is the first step of the roadmap item that prices it, and the certified-implementation criterion from the layered-game-semantics line is the named candidate for the soundness note the crate currently lacks.
...
The gates crate is a standalone analyzer library (no workspace dependencies) with seventeen commands, including the contract-grammar gate (the `# Contract`/`# Adequacy` doc discipline), the graph-boundary fence, and static task plans for the merge and push tiers.
