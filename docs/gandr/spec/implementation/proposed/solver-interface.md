...
* **A dependency-footprint mechanism exists**, at item granularity rather than per constraint: `gandr-core-incrementality`'s `footprint` module captures the ambient-context names a lowered item read, and over-approximates by marking a footprint opaque on any node it cannot represent ([[../incremental-pipeline]]).
...
2. **The tree**, for every as-built claim: `gandr-core-checker`'s `subtype`, `grade`, `effect` and `machine` modules, `gandr-core-incrementality`'s `footprint` and `region` modules, and a workspace-wide check that no crate carries SMT, refinement, or solver-backend machinery.
