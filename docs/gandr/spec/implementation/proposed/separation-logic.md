...
_Where gandr stands today._ The candidate counterpart is internal-wire freshness, marked **plausible** and not established, and the binder that would carry it does not exist: the `feed` statement spells a cycle at the surface but is parse-and-decline, so no internal-wire binder is built ([[../circuit-terms]]).
...
**The polarization substrate on cells predates this line and needs nothing from it.** A cell carries derived per-metavariable variance and linearity metadata, identified by hole name across the faces, with the variance joined across both faces and the linearity counted per `(name, category)` pair ([[../../metatheory#Interchange, by layer]]).
**What the substrate supports is a design, and it is not the reading the prototype built.** A variance-driven polarized reading is one design the substrate would support; `match_footprint` consults no cell metadata at all and derives its three classes from the firing instance instead — the write class by **grafting** (hold a node's immediate children equal to the rewritten term's and ask whether the node still agrees), and the read-against-frame split by **probe perturbation** (substitute each distinct subterm at the address and ask whether the firing survives).
The two are different axes as well — producer-against-consumer variance, against read-against-write access — so neither is evidence for the other.
**An earlier revision of this row said the metadata is what makes a polarized reading of a match image possible at all**, and that causal clause is narrowed here rather than dropped quietly (owner ruling, 2026-08-08): a row in this table records what is built, and the clause described a different implementation than the one that exists.
**Verified against the tree at write time**, at `CellMeta`, `CellVarMeta`, and `CellVariance` in the `sequent` module and at `match_footprint`'s own body in the `footprint` module.
