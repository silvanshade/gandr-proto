...
Two of the polarized-only rows are attributed to the overlap enumerator counting a metavariable position as a composition seam rather than to polarization, and the suite separates them with a fixture whose only overlap is a hole seam, because a measurement that conflates the two is worthless.
