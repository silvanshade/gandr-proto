...
Two of the polarized-only rows are refusals the guard reports at a metavariable position the overlap enumerator counts as a composition seam, and **exactly one of the two is caused by that gap rather than by polarization**: the suite pins each pair's whole overlap family, and only the hole-seam row's family is bare metavariables throughout, so repairing the enumerator would stop the guard refusing that pair while the other row — whose family also carries a genuine composition seam — stays refused either way.
