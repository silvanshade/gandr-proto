# Ornaments over the description universe — what the gate must supply, and what can be built before it is paid
...
It exists because the owner ordered the question answered from a full read of the source, and because the answer turned out to be a schedule rather than a verdict: **the gate the ornament machinery waits on is the gate the n-sorted signature surface (the `sign` block) also had to pass — the sort-indexed description universe — so the gate is paid once**, and the requirements below are design inputs for that single payment.
**The first installment of that payment has landed**: the signature unification put the sort index on the description universe and the `sign` block on the surface ([[../../surface-language/signatures]]), so the ledger below divides into the rows that landing discharged and the rows still owed.

* Status: **proposed, and deferred on the unpaid remainder of the shared gate.** The deferral is not a decline and not a scope cut; it is an import order shared with the signature generalization.
  The sort index is paid and no other required construct is, so the ornament universe still has no statement — at minimum the reindexing frame and the equality-constraint former have to join it.
...
**gandr's description universe carries that index, verified at the symbol.** `Code::Var` is the only former that counts as a recursive occurrence, and it is **not** nullary: it is `Var(Name)`, naming the declared sort the occurrence targets, and its documented contract calls that argument the sort index of the description universe (`theory-levitation`'s `code` module).
The sort set it indexes into is `SignDesc.sorts` — one `SortDesc` per declared sort, the degenerate `data` / `codata` block contributing exactly one — and every constructor names the sort it targets in `CtorDesc.result` (`theory-levitation`'s `desc` module).
`check_desc` polices the index rather than assuming it: a duplicate sort name, a constructor result outside the declared set, and a `var` at an undeclared sort are each declined by name (`theory-levitation`'s `wellformed` module).
An n-sorted mutually-recursive signature is therefore **representable at this universe** — the capability [[#ornament-requirement-01]] names, supplied in the tagged presentation rather than the source's curried one [ibid., def 5.8, def 5.12], and within one polarity, since `check_desc` also declines a sort whose polarity disagrees with its declaration's.

A reference from a declared datatype to **another signature's** datatype is still a `Code::Field` over a symbolic `ValueTypeRef::Ctor`, documented as a **non**-recursive leaf.
What the index changed is the case _inside_ one signature: a recursive occurrence at another declared sort of that same signature is a `Code::Var` at that sort, and `check_desc` declines a `var` at any sort the signature does not declare — so at the universe a mutual recursion within a signature is no longer indistinguishable from an unrelated external type reference.

**The code grammar carries two sort vocabularies, and their relationship is undecided.** `Code::Var`'s argument is a datatype sort drawn from the signature's own sort set; `Code::Bind` names an atom sort, a binder's sort rather than a datatype's, validated against nothing and reachable from no surface field today.
Whether the index set `I` reuses, collides with, or stays disjoint from that atom-sort vocabulary is a design input the sort index's landing did not settle.

**The landing reached the universe and not the two layers on either side of it, which is where the remaining single-sortedness lives.**

_Below it, the decoder._ The stage-1 μ decoder reads the single-sort fragment only: it declines a description declaring several sorts, and declines a `var` at any sort but the decoded description's own, each by name (`theory-levitation`'s `decode` module).

_Above it, the elaborator — and this is the sharper bound._ **No surface form in this tree yet produces a multi-sorted description carrying constructors.** A `sign` block presents sorts, operations, and rules only: its constructor list stays empty by construction and its item-level `data` member is declined as retired (`surface-engine`'s `circuit` module).
A nested `data` / `codata` generator block takes the degenerate single sort, never the multi-sorted builder, fixes every constructor's result to the family's own name, and emits a `Code::Var` only where a field's type head is the block's own (`surface-engine`'s `desc_elab` module).
So the n-sorted mutually-recursive signature the universe admits is one **no source text can currently write**, and the two-sorted example that exercises the sorting discipline is built directly in Rust, in `theory-levitation`'s `wellformed` tests.
...
**So the deferral has a concrete condition, and one half of it is discharged.** The sort index landed with the signature unification, which is the landing the n-sorted `sign` block required at the universe; the second half — the equality former the constructions insert, [[#ornament-requirement-06]] — has not, and the remaining rows price what an ornament universe over it would still need on top of both.
...
Two payoff facts from the source's machinery block ride with this pair, carried here so the importer meets them where they bind (the signature-unification lane's orn-lead-04 disposition): **iterating reornamentation terminates** — the reornament of a reornament is computationally trivial, so the tower closes rather than regressing [ibid., rem 8.66] — and **for containers the refinement functor is itself polynomial and ornaments the base** [ibid., prop 8.100], which is what lets the refinement story stay inside the container discipline the sort-indexed description universe is built on.
...
_Where gandr stands today — discharged at the universe, not yet at the language._ `Code::Var(Name)` names the declared sort a recursive occurrence targets, `SignDesc.sorts` is the declared sort set, `CtorDesc.result` is each constructor's target sort, and `check_desc` declines a duplicate sort name, a foreign result sort, a `var` at an undeclared sort, and a sort whose polarity disagrees with its declaration's (`theory-levitation`'s `code`, `desc`, and `wellformed` modules).
`Code::is_recursive` still reports only whether a code _contains_ a `Code::Var` anywhere, and cross-datatype references that are not recursive occurrences remain `Code::Field` leaves over `ValueTypeRef::Ctor`.

**The two layers around the universe are the residue, and the row's own failure clause names the second of them.** The stage-1 μ decoder still reads the single-sort fragment, declining a multi-sorted description and a foreign-sort `var` by name (`theory-levitation`'s `decode` module).
And no surface form yet elaborates to a multi-sorted description with constructors — a `sign` block accumulates none and declines the retired item-level `data` member, while a nested generator block takes the degenerate single sort and recurses only at its own name (`surface-engine`'s `circuit` and `desc_elab` modules) — so "the n-sorted `sign` block can be expressed" holds of the description universe and not yet of anything a source file can say.
...
* **The tagged-description normal form is already the as-built shape, and it was the right call for reasons the source states.** The source's tagged form `tagDesc = (E : EnumU) × π E (λ_. Desc)` [@dagand-2013-cosmology, def 4.23] is `SignDesc`'s constructor table — `CtorDesc { name, code, result, attrs }` (`theory-levitation`'s `desc` module) — and the source's rationale for enforcing constructor form, that it "ease[s] the implementation of datatype transformations" [ibid., ¶ 4.24], is the ornament argument banked in advance.
* **Both the operation layer and the constructor layer are container-shaped, and the `sign` block made them agree.** `BridgeArity` carries inputs, per-monomial factor counts, a source map, a destination map, and outputs (`theory-levitation`'s `arity` module) — a container in the source's sense [ibid., def 5.53], with the correspondence exhibited rather than asserted: the destination map's fibres give the operations over the output ports, the factor counts give each operation's arity set, and the source map is the sort map into the input ports.
  The asymmetry the ornament read exposed — a constructor layer with no sort map and a merely reserved output-sort slot — is the one the signature unification closed: `CtorDesc.result` is now the real output-sort slot, and `CtorDesc::arity` reads a constructor as a single-output bridge arity whose output port sits at that result sort and whose input ports sit at the leaf types the payload names, a declared sort for a `Code::Var` leaf and a parameter, primitive, or foreign head for a `Code::Field` one.
  The agreement is therefore exhibited rather than asserted — though nothing enforces it yet: `check_desc` runs its arity check over the operation members only, and no caller outside the crate's own tests reads a constructor's arity.
* **An extension-only forgetful fold is host-implementable today — stated as buildable, not as recommended.** An `insert`-only ornament over the current fragment is a per-constructor field deletion, and the forgetful projection is a structural fold over `Code` [ibid., def 8.32].
  Building it before [[#ornament-requirement-02]] lands would fix an interface that reindexing then changes; it buys only the examples that need no index refinement [ibid., ex 8.22, ex 8.25].
...
**Two rows are priced against `theory-levitation` alone, and the core carries type formers they do not account for.** [[#ornament-requirement-03]] and [[#ornament-requirement-06]] ask for a dependent Σ and a propositional equality; `core-checker`'s value-type universe already has a dependent `Sigma` former and a `Path` identity type, and the first-order code fragment can name neither.
Both rows are therefore narrower than they read — what is missing is a **code** former over machinery the core may already supply — and this document has not re-derived either row against that crate.
...
**One as-built claim rests on read code rather than read prose, and it is the load-bearing one.** That the as-built universe expresses an n-sorted signature is exhibited by `Code::Var`'s sort argument, by `SignDesc.sorts` and `CtorDesc.result`, and by the sort-membership declines `check_desc` emits — and that no source text yet elaborates to one is exhibited by the empty constructor list and the retired-member decline in the `sign` route, and by the single-sort constructor of the generator-block route.
