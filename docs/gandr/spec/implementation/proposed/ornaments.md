...
An n-sorted mutually-recursive signature is therefore expressible — the capability [[#ornament-requirement-01]] names, supplied in the tagged presentation rather than the source's curried one [ibid., def 5.8, def 5.12].
...
What the index changed is the case _inside_ one signature: a recursive occurrence at another declared sort of that same signature is a `Code::Var` at that sort, and `check_desc` declines a `var` at any sort the signature does not declare — so a mutual recursion within a signature is no longer indistinguishable from an unrelated external type reference.

**The code grammar carries two sort vocabularies, and their relationship is undecided.** `Code::Var`'s argument is a datatype sort drawn from the signature's own sort set; `Code::Bind` names an atom sort, a binder's sort rather than a datatype's.
...
**What the landing did not reach is the decoder.** The stage-1 μ decoder reads the single-sort fragment only: it declines a description declaring several sorts, and declines a `var` at any sort but the decoded description's own, each by name (`theory-levitation`'s `decode` module).
Cross-sort recursion is representable and checkable, and not yet decodable — which bounds what an ornament built over these codes could be run against, not what it could be stated over.
...
Decoding is the part the landing did not reach: the stage-1 μ decoder reads the single-sort fragment only, declining a multi-sorted description and a foreign-sort `var` by name (`theory-levitation`'s `decode` module).
