...
* five pre-reboot records were dispositioned by the phase-2 sweep, and **that sweep's method does not hold**: it credited a corpus section whose _subject_ matched a record's subject without checking that the section made the record's _claims_.
  Each disposition is therefore being re-measured by reading the record against the sections said to carry it, and the results so far are of three kinds: one disposition stands, two records turned out to be substantially uncarried and are absorbed in full, and two re-measures are still owed.
  ...
  The **incremental-pipeline** record is owed the same re-measure, and its checkpoint credit rests on the claim just measured false;
