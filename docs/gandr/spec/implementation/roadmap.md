...
* five pre-reboot records were dispositioned by the phase-2 sweep, and **that sweep's method does not hold**: it credited a corpus section whose _subject_ matched a record's subject without checking that the section made the record's _claims_.
  Each disposition is therefore being re-measured by reading the record against the sections said to carry it, and the results so far split three ways.
  ...
  The **typing-machine** and **incremental-pipeline** records are owed the same re-measure, and the typing-machine one is partly done: the checkpoint discipline credited to the incremental lane and [[performance-architecture]] is the transactional staging overlay on the normalizer arena, which is a different mechanism from the machine's checkpoint dependency footprint and its trail-depth validity rule.
  The one detail the sweep declined outright — the typing-machine record's full frame inventory, declined as reference duplication on the ground that the machine's own source carries it — is **reversed** (`gandr-fid.14.7-answer-10`): measured against `crates/core-checker/src/machine.rs`, that ground holds for the built core and fails for the frames belonging to features the checker does not build, so the inventory is carried in full in the absorbing document, with a built-versus-designed status per frame rather than split across two registers;
