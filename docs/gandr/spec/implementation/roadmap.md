...
- **The engines' first external consumer**: `theory-computads` has one, at the **elaboration face only**, and it is not the completion machinery.
  Verified against the tree 2026-08-08: `surface-engine`'s library calls `gandr_theory_computads::elaborate_data_desc` to build a `CellStore` from each description (`crates/surface-engine/src/desc_cells.rs`, landed 2026-08-01), and that plus `CellCount` in its own test is the whole external surface; nothing outside the engines' own tests reaches overlap enumeration, completion, rewriting, shift equivalence, or tracelets, and nothing outside its own tests reaches `theory-virtual-doctrines` at all.
  That seam has a downstream driver as well as a direct caller: `gandr-surface-corpus`'s library reaches it for the `desc` inspection mode through `gandr_surface_engine::desc_cells`, without a dependency on `theory-computads` of its own (`crates/surface-corpus/src/lib.rs`).
  `theory-circuit-algebras` holds a manifest edge to `theory-computads` and no code use, by the ruling that minted it ([[circuit-terms#Matching, normalization, and the crate boundary]]) — the edge is what forbids the reverse one, not a consumer.
  So the integration phase is still a demo obligation over the completion machinery rather than construction, and the milestone this residual is watching for is the first consumer of _that_.
...
- five pre-reboot records were dispositioned by the phase-2 sweep, and **that sweep's method does not hold**: it credited a corpus section whose _subject_ matched a record's subject without checking that the section made the record's _claims_.
  Each disposition has now been re-measured by reading the record against the sections said to carry it, and the results are of three kinds: one disposition stands, two records turned out to be substantially uncarried and are absorbed in full, and two more are measured with their absorptions owed.
  The **kernel-boundary** record's retirement stands: its content is verified as substantially carried by this track's trusted-base account (the TCB wall and naming rule, the single admission choke point with its unforgeable checked-id and warned bypass, the canonical level algebra and landmark posets, the export and replay disciplines, the unsoundness threat model, effects never in checked conversion).
  The **type-system** record was credited to this track's checked-language section and the surface track's spellings; re-measured, those carry the built fragment's grammar in prose and none of the record's rule-level content, so the record is absorbed in full as [[type-system]] and the sections that were credited now link to it.
  The **effects, control, and shell** record was credited to this track's runtime-host account, which dispatches on an operation name — while the record's effect calculus is a row grammar with operation and handle rules and row unification as a solver domain, none of which was in the corpus — so the record is absorbed in full as [[effects-and-control]], which also found that both of its typing halves are built while its two soundness disciplines, the open row and the linear stack, are not.
  The **typing-machine** record was credited to this track's checker account and to the checkpoint disciplines of the incremental lane and [[performance-architecture]]; re-measured, the checker account carries three of its claims and the checkpoint credit is wrong — what those documents carry is the transactional staging overlay on the normalizer arena, a different mechanism from the machine's checkpoint dependency footprint and its trail-depth validity rule — so the record is absorbed in full as [[typing-machine]].
  The one detail the sweep declined outright — that record's full frame inventory, declined as reference duplication on the ground that the machine's own source carries it — is **reversed** (`gandr-fid.14.7-answer-10`): measured against `crates/core-checker/src/machine.rs`, that ground holds for the built core and fails for the frames belonging to features the checker does not build, so the inventory is carried in full with a built-versus-designed status per frame rather than split across two registers.
  The **incremental-pipeline** record was credited to the same two places, and both credits fail: the surface-pipeline account is the _parse_ pipeline, whose stages have no counterpart in an edit-to-render loop, and the checkpoint credit is again the staging overlay.
  What does carry the record's reuse rule arrived from the other side — [[typing-machine]] states two of its three conditions in the machine's own register, and the third is pipeline-level because it is the diff that establishes an unchanged region.
  The re-measure also found that the record's subject is **built and unspecified**: the structural diff, the checkpoint and footprint modules of both the engine and the checker, and the order-maintenance structure all exist, and thirty-six doc comments across twenty files cite the record by a path no gandr reader can open;
