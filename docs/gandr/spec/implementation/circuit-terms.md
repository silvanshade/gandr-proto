...
16. **circuit-terms-question-16** — **which of gandr's structures is a device, and which is a resource?** One runtime object means one sequentialization and makes every cell depend on every other [@earnshaw-sobocinski-2023-string-diagrammatic-trace-theory, prop 34], which is the opposite of what the disconnection axis is for.
    ...
    > Both sources are read at that claim only and are **unread at theorem grade for this use**; the read and the incorporation scoping are filed on the tracker, and nothing in this direction binds before that read reports.
...
The route cost is asymmetric and decided the fork at [[#circuit-terms-question-20]]; the spider triple's bead count is computable near-linearly on the representation by union-find over the port bijection, with the identification's own correction pending its sign-off.
