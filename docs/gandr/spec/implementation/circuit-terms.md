...
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------ |
| IL grammar (`core-sequent/src/il.rs`)                   | `ProducerNode::Ctor`, `ConsumerNode::Dtor`, `CommandNode::Prim`, and `CommandNode::Jump` each carry `cs: Box<[ConsumerId]>`; the first three **declare** that count at their head — `CtorTag::consumer_arity`, `DtorTag::consumer_arity`, `PrimOp::consumer_arity`, over a `ConsumerArity` wrapper distinct from the producer side's | **representable and declared** — no format break owed                          |
| typed-IL checker (`core-sequent/src/check.rs`)          | every consumer list is held to the arity its own head declares, through one `CheckError::ConsumerArity { head, expected, found }`; a `Jump`'s arguments are walked uncounted, its head being a definition name rather than a tag                                                                                                     | **tag-declared** (2026-08-08); the `Jump` seam is the residue                  |
...
The summary a reader should take away: **the sequent layer now declares many-out rather than merely tolerating it.** When this table was first written its support was exactly that a field is a `Vec` — real, because no serialized format or node layout has to change, and strictly weaker than support, because nothing stated or policed what the field was allowed to hold.
Since [[#circuit-terms-rung-06]] the count is declared at the head and read by the checker, so a head admitting several return continuations passes the typed IL with no further change there; what is still missing at this layer is a head that declares more than one and a machine step that runs it.
The cell alphabet — the layer where circuit structure would actually earn something — still cannot express it at all.
...
**It is priced now, and the answer is "little" (2026-08-08):** the machinery the first question needs on the monogamous fragment is one boundary-anchored traversal plus a per-component minimization, built at the as-built block below, and it is small enough that it does not by itself justify a crate — the crate was justified by the three faces together.
...
**As built (2026-08-08): the edge orientation is selected, and on the monogamous fragment the decision is a traversal rather than a search.** `gandr-theory-circuit-algebras`'s `normal_form` face canonicalizes a diagram by renumbering it: the declared input ports take the lowest canonical wire numbers in order, then the declared output ports, then a cursor drains the numbered wires in canonical order, visiting each wire's incident hyperedges and numbering each visited hyperedge's sources before its targets.
**The selection's reason is that monogamy makes the edge cut choice-free while leaving the vertex cut with a choice to break.** Removing a wire leaves two sides told apart by _direction_ — a wire has at most one producer and at most one consumer — so "recurse into the two components the edge's removal leaves" is "step to the producer, then to the consumer"; removing a generator leaves as many pieces as it has ports, and sequencing those pieces needs an order the corolla shape does not supply, which in gandr would have to be the port order the edge orientation already consumes ([[#circuit-terms-question-02]]).
An interface is a list of _wires_, so the anchoring datum a cospan carries is already an edge datum, and the traversal seeds on it without converting anything.
The mixed style stays bracketed rather than dismissed: what would move the selection toward it is a generator whose ports are not totally ordered, which [[#circuit-terms-question-02]] currently declines.

**The source's non-confluence caution is answered rather than inherited, and the reason is a change of reading.** [[../metatheory/roadmap#meta-spike-09|meta-spike-09]] records that the same source's rewriting system is non-confluent at exactly the cut symmetry, with five distinct normal forms for one tree, so a decomposition shape owes its own confluence argument.
That is an argument about producing a normal form by **rewriting toward** it.
The selected linearization is produced by a deterministic **traversal**, so there is no rewriting relation to be confluent: uniqueness is by construction, not by convergence.
...
* **The `Rigid` correspondence is exact, and the metatheory's own instance stays owed.** The diagram view is the setoid, the canonical form is the splitting `Nf`, canonicalization is `nf`, reading a canonical form back as a diagram is `emb`, the returned relabelling is `canon-≈` as a **checkable witness**, and the derived equality on canonical forms is `_≟_`; `canon-sound` is derived in `Rigid` from `canon-≈`, so the decision procedure returns the shared form together with **both** relabellings rather than a bare verdict, and nothing is handed over as an unguarded equality fast path.
  What this does **not** discharge is [[../metatheory/roadmap|the metatheory roadmap]]'s `Rigid.canon-sound` at the circuit rung, which is a normal form on the carrier's construction terms over merger and contraction — permutations outermost, ordered tree monomials, unique minimal representative — a different object whose agreement with this one is an owed question rather than an assumption.
...
16. **circuit-terms-question-16** — **which of gandr's structures is a device, and which is a resource?** One runtime object means one sequentialization and makes every cell depend on every other [@earnshaw-sobocinski-2023-string-diagrammatic-trace-theory, prop 34], which is the opposite of what the disconnection axis is for.
    ...
    > **Both sources are read at theorem grade for this use, and what arrives is the apparatus rather than the logic riding on it**: what survives contact with gandr's carrier is the template and cobordism apparatus, while the separation logic — separated states, the separating conjunction over a permission monoid, locks, and the data-race half of soundness — is declined at the door.
...
| a multi-consumer command the typed-IL checker admits or refuses | the tag-declared arity, replacing the hard-coded one — **written** (2026-08-08) as `core-sequent`'s `check::tests` consumer-arity refusals, the internal-fixture form the corpus rule asks for while no surface emits one |
