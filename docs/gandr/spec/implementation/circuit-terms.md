...
So this dissolves the convexity question **for the acyclic case only**, and says nothing about a re-closed delay loop, whose target is by construction not acyclic — that stays [[#circuit-terms-spike-08|circuit-terms-spike-08]]'s question, untouched.
...
> Either the disjointness test is strengthened to a convexity-stable one, or the shift quotient is fenced to the fragment where convexity cannot be broken (left-connected left-hand sides are the published such fragment, and gandr's as-built ones are inside it).
...
15. **circuit-terms-question-15** — **is gandr's disjointness test convexity-stable?** Under convex rewriting, two rule applications on **disjoint** sets of hyperedges can block one another, because one can create a directed path that destroys the other's convexity [@bonchi-gadducci-kissinger-sobocinski-zanasi-2022-string-diagram-rewriting-ii, ex 45]. gandr's shift equivalence commutes adjacent applications at disjoint positions with trivial overlap, and that quotient is TCB-adjacent.
    ...
    **What follows for this question is that the fence is free today and stops being free the moment multi-output or disconnection reaches the rule layer** — so the alphabet decision is a route decision as well, and the choice between the two answers may not be settled by pointing at today's inventory.
...
* Does an analogous pair exist over gandr's cell fragment once patterns are circuit-shaped, or does the cut-rooted left-hand-side discipline already exclude it?
  **Half answered**: the discipline does exclude it for the as-built alphabet, because those left-hand sides are strongly connected and left-connectedness makes a match on an acyclic target automatically convex ([[#The correspondence at gandr's own rung, at theorem grade]]); what stays open is what survives multi-output and disconnection.
...
The honest default until it runs is that the shift quotient is warranted only on the fragment where matches cannot be non-convex, which today is the whole cell store and after the alphabet change is not — and the reason it is the whole cell store is now stated rather than assumed.
