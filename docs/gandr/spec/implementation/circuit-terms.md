...
**gandr's carrier already says so, and this is the check that settles it.** `Match` pairs every source with exactly one partner and every sink is hit exactly once (the carrier record, machine-checked; migrated — the corpus README's migration banner) — so no wiring datum in the carrier can name two targets for one source, and fan-out is no more expressible there than fan-in.
