...
- **surface-engine / surface-render-remote**: the parse's recovery obligations become a live report surface, replacing the reserved slot that always serialized as `[]`.
  A lowering now carries the melder's obligation buffer verbatim (`lower::Lowered::obligations`) — the parse stays the only authority on what a source had to repair, and nothing between it and the report adds, drops, or re-spans a row — and `diag::obligations` projects that buffer into typed `diag::ObligationReport` rows carrying the obligation class and its exact responsible byte span, ordered by start, then end, then class severity.
  The report envelope's `SCHEMA_VERSION` moves to `3` on the same precedent the `marks` slot set: a reserved slot becoming populated typed data changes meaning.
  The class vocabulary is published once, as `present::ObligationClass` in the leaf wire crate, so the agent-stream JSON and the render bus name the same eight classes; `gandr_surface_parser::Oblig` remains the semantic authority the melder minimizes over, and `diag::obligation_class` is the single exhaustive crossing between them, so a class added upstream is a compile error rather than a row that loses its class.
  `remote::obligation_cards` is the producer side — the live path from a report into `present::ObligationCard` and `wire::ReportView::obligations` — and `remote::server_caps` advertises the one capability that path backs.
  The wire's `WIRE_SCHEMA_VERSION` moves to `2`: the `obligations` capability meant "obligation deltas are populated" for a delta channel that was never built, and now means the whole-frame report view carries typed rows (`ObligationDeltas` is renamed `ObligationRows` with it).
  A clean source reports an empty row set on every one of these surfaces, and the diagnostic, goal, mark, and attribute surfaces are unchanged — every report golden moved by exactly its schema-version line.
...
- **core-checker**: the normalizer's semantic arena holds **handles**, never terms — a correction to the landing above, made at review.
  The term face had held a reference-counted source value and a closure a reference-counted computation, both inside arena records, which reintroduced the abstract syntax tree's recursive-drop ownership: teardown was flat only while some other holder still owned the term, and the deep-term witness passed only because it released the normalizer first.
  Every semantic record now names a node in the flat carrier the crate already ships (`syntax::FlatArena`) — the term face a value node, a closure body a computation node, a reified stack a stack node, a quarantined effect head its own source node — and evaluation, readback, the definitional environment, and the canonical-key walk moved onto node ids with it.
  Teardown is structural rather than maintained: the arena is vectors of nodes with id children, the store is vectors of flat syntax nodes, and drop order stops mattering because nothing in the engine participates in the term's ownership.
  Both deep-term witnesses now release the caller's input **before** the normalizer, with no ordering care taken.
