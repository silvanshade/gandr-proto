...
void print_outcome(std::string_view name, const RunOutcome& outcome)
{
    std::printf(
        "%.*s\t%s\t%lld\t%lld\n",
...
void print_error(std::string_view name, const HostError& error)
{
    std::fprintf(
        stderr,
        "%.*s: %.*s: %s\n",
...
[[nodiscard]] int interpret_samples()
{
    int status = EXIT_SUCCESS;
    for (const Sample& sample : all_samples()) {
        const Expected<RunOutcome> outcome = interpret_image(sample.image);
        if (!outcome.has_value()) {
            print_error(sample.name, outcome.error());
            status = EXIT_FAILURE;
            continue;
...
[[nodiscard]] int report_timings()
{
    int status = EXIT_SUCCESS;
    for (const Sample& sample : all_samples()) {
        const Expected<std::pair<RunOutcome, RunTiming>> timed = compile_and_run_timed(sample.image);
        if (!timed.has_value()) {
...
[[nodiscard]] int dump_module(std::string_view name, bool lowered)
{
    for (const Sample& sample : all_samples()) {
        if (sample.name != name) {
            continue;
        ...
        if (lowered) {
            ...
            const Expected<void> result = verify_module(module->get());
            if (!result.has_value()) {
                print_error(name, result.error());
                return EXIT_FAILURE;
            }
...
[[nodiscard]] int write_seeds(std::string_view directory)
{
    ...
    for (const Sample& sample : all_samples()) {
        ...
        const std::filesystem::path path =
            std::filesystem::path(directory) / (std::string(sample.name) + ".bin");
...
int main(int argc, char** argv)
{
    ...
    const std::string_view mode = arguments.front();
