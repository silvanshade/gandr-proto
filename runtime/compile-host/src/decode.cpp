...
struct Cursor
{
    ...
    /// Reads one byte.
    [[nodiscard]] std::optional<std::uint8_t> take_u8() noexcept
    {
        if (rest.empty()) {
            return std::nullopt;
        }
        const std::uint8_t byte = rest.front();
        rest = rest.subspan(1);
        return byte;
    }
    ...
    [[nodiscard]] std::optional<T> take_le() noexcept
    {
        if (rest.size() < Width) {
            return std::nullopt;
        }
        ...
        for (std::size_t byte_index = 0; byte_index < Width; ++byte_index) {
            accumulated |= static_cast<std::uint64_t>(rest[byte_index])
...
void put_le(std::vector<std::uint8_t>& bytes, std::uint64_t value)
{
    for (std::size_t byte_index = 0; byte_index < Width; ++byte_index) {
        bytes.push_back(
            static_cast<std::uint8_t>((value >> (8 * static_cast<std::uint64_t>(byte_index))) & 0xFFU));
    }
}
...
std::optional<Image> decode_image(std::span<const std::uint8_t> bytes)
{
    Cursor cursor{bytes};

    const std::optional<std::uint8_t> version = cursor.take_u8();
    if (!version.has_value() || *version != image_version) {
        return std::nullopt;
    }

    const std::optional<std::uint16_t> node_count = cursor.take_le<std::uint16_t, 2>();
    if (!node_count.has_value() || *node_count == 0
        || static_cast<std::size_t>(*node_count) > max_image_nodes) {
        return std::nullopt;
    }
    ...
    for (std::uint16_t node_index = 0; node_index < *node_count; ++node_index) {
        const std::optional<std::uint8_t> kind = cursor.take_u8();
        const std::optional<std::uint8_t> tag = cursor.take_u8();
        const std::optional<std::uint32_t> binder = cursor.take_le<std::uint32_t, 4>();
        const std::optional<std::uint64_t> literal = cursor.take_le<std::uint64_t, 8>();
        const std::optional<std::uint8_t> operand_count = cursor.take_u8();
        if (!kind.has_value() || !tag.has_value() || !binder.has_value() || !literal.has_value()
        ...
        for (std::uint8_t operand_index = 0; operand_index < *operand_count; ++operand_index) {
            const std::optional<std::uint32_t> operand = cursor.take_le<std::uint32_t, 4>();
...
std::vector<std::uint8_t> encode_image(const Image& image)
{
