...
[[nodiscard]] std::optional<std::int64_t> read_word(
...
std::optional<std::string> render_value(std::span<const std::int64_t> heap, std::int64_t root)
{
    ...
    while (!pending.empty()) {
        const RenderStep step = std::move(pending.back());
        pending.pop_back();
        ...
        const std::optional<std::int64_t> tag_word = read_word(heap, step.cell);
        if (!tag_word.has_value()) {
            return std::nullopt;
        }

        const auto push_text = [&pending](std::string text) {
            pending.push_back(RenderStep{.is_cell = false, .cell = 0, .depth = 0, .text = std::move(text)});
        };
        const auto push_cell = [&pending, &step](std::int64_t offset) {
            pending.push_back(
                RenderStep{.is_cell = true, .cell = offset, .depth = step.depth + 1, .text = {}});
        };
        ...
        case CellTag::Int: {
            const std::optional<std::int64_t> payload = read_word(heap, step.cell + 1);
        ...
        case CellTag::Unit:
            ...
            break;
        case CellTag::Pair: {
            const std::optional<std::int64_t> first = read_word(heap, step.cell + 1);
            const std::optional<std::int64_t> second = read_word(heap, step.cell + 2);
...
std::size_t heap_words_for(const Image& image) noexcept
{
...
std::size_t node_allocation_words(NodeKind kind, CtorTag tag) noexcept
{
    switch (kind) {
    case NodeKind::Lit:
        return 2;
