...
/// - panics: none.
[[nodiscard]] std::optional<std::size_t> fixed_operand_count(NodeKind kind) noexcept
{
    switch (kind) {
    case NodeKind::Lit:
    case NodeKind::Var:
...
std::uint32_t operand_binder_offset(NodeKind kind, std::size_t slot) noexcept
{
    switch (kind) {
    case NodeKind::Bind:
        return slot == 1 ? 1U : 0U;
...
bool image_is_wellformed(const Image& image) noexcept
{
    ...
    for (std::size_t index = image.nodes.size(); index-- > 0;) {
        const Node& node = image.nodes[index];

        const std::optional<std::size_t> required = fixed_operand_count(node.kind);
        const std::size_t expected =
            required.has_value() ? *required : std::size_t{ctor_arity(node.tag)};
        ...
        for (std::size_t slot = 0; slot < node.operands.size(); ++slot) {
            const NodeIndex operand = node.operands[slot];
