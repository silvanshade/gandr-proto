...
public:
    ...
    [[nodiscard]] const std::string& collected() const noexcept { return collected_; }
...
Expected<void> verify_module(mlir::ModuleOp module)
{
    DiagnosticCollector collector(*module.getContext());
    if (mlir::failed(mlir::verify(module))) {
        return host_error(
            ErrorKind::VerifierRejected,
            collector.collected().empty() ? std::string("module failed verification")
                                          : collector.collected());
    }
    return Expected<void>{};
}

Expected<void> optimize_module(mlir::ModuleOp module, Optimization optimization)
{
    ...
    if (mlir::failed(manager.run(module))) {
        return host_error(
            ...
            collector.collected().empty() ? std::string("optimization pipeline failed")
                                          : collector.collected());
    }
    return Expected<void>{};
}

Expected<void> lower_module(mlir::ModuleOp module, Optimization optimization)
{
    const Expected<void> optimized = optimize_module(module, optimization);
    if (!optimized.has_value()) {
        return optimized;
    }

    const Expected<void> structural = lower_dialect_operations(module);
    ...
    if (mlir::failed(mlir::verify(module))) {
        return host_error(
            ...
            collector.collected().empty() ? std::string("lowered module failed verification")
                                          : collector.collected());
    }
    ...
    if (mlir::failed(manager.run(module))) {
        return host_error(
            ErrorKind::ConversionFailed,
            collector.collected().empty() ? std::string("conversion to the LLVM dialect failed")
                                          : collector.collected());
    }
    ...
    if (mlir::failed(mlir::verify(module))) {
        return host_error(
            ErrorKind::ConversionFailed,
            collector.collected().empty() ? std::string("converted module failed verification")
                                          : collector.collected());
    }
    ...
    if (!module.lookupSymbol<mlir::LLVM::LLVMFuncOp>(std::string(entry_point_name))) {
        return host_error(
            ErrorKind::ConversionFailed,
            "conversion left the entry point outside the LLVM dialect");
    }
    return Expected<void>{};
}
